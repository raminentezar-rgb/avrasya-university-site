...
class HaritaKadastroDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='harita_kadastro',
            yayinda=True
        )

class HaritaKadastroDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته نقشه‌برداری و کاداستر"""
    
    objects = HaritaKadastroDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Harita ve Kadastro Duyurusu"
        verbose_name_plural = "Harita ve Kadastro Duyuruları"

class HaritaKadastroEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/harita_kadastro/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Harita ve Kadastro)"
        verbose_name_plural = "Etkinlikler (Harita ve Kadastro)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('harita_kadastro:etkinlik_detay', args=[self.slug])
...
class HaritaKadastroDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/harita_kadastro/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Harita ve Kadastro)"
        verbose_name_plural = "Ders Programları (Harita ve Kadastro)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class HaritaKadastroFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Harita Kadastro)"
        verbose_name_plural = "Faaliyet Grupları (Harita Kadastro)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class HaritaKadastroFaaliyet(models.Model):
    grup = models.ForeignKey(HaritaKadastroFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Harita Kadastro)"
        verbose_name_plural = "Faaliyetler (Harita Kadastro)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class HaritaKadastroFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(HaritaKadastroFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/harita_kadastro/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Harita Kadastro)"
        verbose_name_plural = "Faaliyet Görselleri (Harita Kadastro)"
        ordering = ['sira', 'id']
