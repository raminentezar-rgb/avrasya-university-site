# app_name: is_sagligi_guvenligi/models.py
...
class IsSagligiGuvenligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='is_sagligi_guvenligi',
            yayinda=True
        )

class IsSagligiGuvenligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته ایمنی و سلامت شغلی"""
    
    objects = IsSagligiGuvenligiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İş Sağlığı ve Güvenliği Duyurusu"
        verbose_name_plural = "İş Sağlığı ve Güvenliği Duyuruları"

class IsSagligiGuvenligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/is_sagligi_guvenligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İş Sağlığı ve Güvenliği)"
        verbose_name_plural = "Etkinlikler (İş Sağlığı ve Güvenliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('is_sagligi_guvenligi:etkinlik_detay', args=[self.slug])
...
class IsSagligiGuvenligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/is_sagligi_guvenligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İş Sağlığı ve Güvenliği)"
        verbose_name_plural = "Ders Programları (İş Sağlığı ve Güvenliği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IsSagligiGuvenligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Is Sagligi Guvenligi)"
        verbose_name_plural = "Faaliyet Grupları (Is Sagligi Guvenligi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class IsSagligiGuvenligiFaaliyet(models.Model):
    grup = models.ForeignKey(IsSagligiGuvenligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Is Sagligi Guvenligi)"
        verbose_name_plural = "Faaliyetler (Is Sagligi Guvenligi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class IsSagligiGuvenligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IsSagligiGuvenligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/is_sagligi_guvenligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Is Sagligi Guvenligi)"
        verbose_name_plural = "Faaliyet Görselleri (Is Sagligi Guvenligi)"
        ordering = ['sira', 'id']
