# app_name: patoloji_laboratuar/models.py
...
class PatolojiLaboratuarDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='patoloji_laboratuar',
            yayinda=True
        )

class PatolojiLaboratuarDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته پاتولوژی آزمایشگاهی"""
    
    objects = PatolojiLaboratuarDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Patoloji Laboratuvarı Duyurusu"
        verbose_name_plural = "Patoloji Laboratuvarı Duyuruları"

class PatolojiLaboratuarEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/patoloji_laboratuar/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Patoloji Laboratuvarı)"
        verbose_name_plural = "Etkinlikler (Patoloji Laboratuvarı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('patoloji_laboratuar:etkinlik_detay', args=[self.slug])
...
class PatolojiLaboratuarDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/patoloji_laboratuar/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Patoloji Laboratuvarı)"
        verbose_name_plural = "Ders Programları (Patoloji Laboratuvarı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class PatolojiLaboratuarFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class PatolojiLaboratuarFaaliyet(models.Model):
    grup = models.ForeignKey(PatolojiLaboratuarFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class PatolojiLaboratuarFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(PatolojiLaboratuarFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/patoloji_laboratuar/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
