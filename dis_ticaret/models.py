# app_name: dis_ticaret/models.py
...
class DisTicaretDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='dis_ticaret',
            yayinda=True
        )

class DisTicaretDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته تجارت خارجی"""
    
    objects = DisTicaretDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Dış Ticaret Duyurusu"
        verbose_name_plural = "Dış Ticaret Duyuruları"

class DisTicaretEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/dis_ticaret/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Dış Ticaret)"
        verbose_name_plural = "Etkinlikler (Dış Ticaret)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('dis_ticaret:etkinlik_detay', args=[self.slug])
...
class DisTicaretDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/dis_ticaret/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Dış Ticaret)"
        verbose_name_plural = "Ders Programları (Dış Ticaret)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class DisTicaretFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class DisTicaretFaaliyet(models.Model):
    grup = models.ForeignKey(DisTicaretFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class DisTicaretFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(DisTicaretFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/dis_ticaret/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
