...
class IsletmeIngilizceDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='isletme-ingilizce',
            yayinda=True
        )

class IsletmeIngilizceDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مدیریت کسب و کار انگلیسی"""
...
class IsletmeIngilizce_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/isletme_ingilizce/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "İşletme (İngilizce) Etkinliği"
        verbose_name_plural = "İşletme (İngilizce) Etkinlikleri"
...
class DersProgramiDosyaIsletmeIngilizce(models.Model):
    DOSYA_TURU_CHOICES = [
        ('pdf', 'PDF'),
        ('word', 'Word'),
        ('excel', 'Excel'),
    ...
    dosya = models.FileField(
        upload_to='ders_programlari_isletme_ingilizce/%Y/%m/%d/',
        verbose_name="Dosya"
    )
    ...
    class Meta:
        verbose_name = "Ders Programı Dosyası (İşletme İngilizce)"
        verbose_name_plural = "Ders Programı Dosyaları (İşletme İngilizce)"
