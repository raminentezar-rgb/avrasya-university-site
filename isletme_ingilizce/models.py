# app_name: isletme_ingilizce/models.py
...
class IsletmeIngilizceDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='isletme_ingilizce',
            yayinda=True
        )

class IsletmeIngilizceDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مدیریت بازرگانی انگلیسی"""
    
    objects = IsletmeIngilizceDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İşletme İngilizce Duyurusu"
        verbose_name_plural = "İşletme İngilizce Duyuruları"

class IsletmeIngilizceEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/isletme_ingilizce/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İşletme İngilizce)"
        verbose_name_plural = "Etkinlikler (İşletme İngilizce)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('isletme_ingilizce:etkinlik_detay', args=[self.slug])
...
class IsletmeIngilizceDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/isletme_ingilizce/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İşletme İngilizce)"
        verbose_name_plural = "Ders Programları (İşletme İngilizce)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IsletmeIngilizceFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IsletmeIngilizceFaaliyet(models.Model):
    grup = models.ForeignKey(IsletmeIngilizceFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IsletmeIngilizceFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IsletmeIngilizceFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/isletme_ingilizce/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
