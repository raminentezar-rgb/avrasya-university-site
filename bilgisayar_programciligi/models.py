# app_name: bilgisayar_programciligi/models.py
...
class BilgisayarProgramciligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='bilgisayar_programciligi',
            yayinda=True
        )

class BilgisayarProgramciligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته برنامه‌نویسی کامپیوتر"""
    
    objects = BilgisayarProgramciligiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Bilgisayar Programcılığı Duyurusu"
        verbose_name_plural = "Bilgisayar Programcılığı Duyuruları"

class BilgisayarProgramciligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/bilgisayar_programciligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Bilgisayar Programcılığı)"
        verbose_name_plural = "Etkinlikler (Bilgisayar Programcılığı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('bilgisayar_programciligi:etkinlik_detay', args=[self.slug])
...
class BilgisayarProgramciligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/bilgisayar_programciligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Bilgisayar Programcılığı)"
        verbose_name_plural = "Ders Programları (Bilgisayar Programcılığı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class BilgisayarProgramciligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class BilgisayarProgramciligiFaaliyet(models.Model):
    grup = models.ForeignKey(BilgisayarProgramciligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class BilgisayarProgramciligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(BilgisayarProgramciligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/bilgisayar_programciligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
