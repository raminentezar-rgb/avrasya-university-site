# app_name: mimari_restorasyon/models.py
...
class MimariRestorasyonDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='mimari_restorasyon',
            yayinda=True
        )

class MimariRestorasyonDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته معماری و مرمت"""
    
    objects = MimariRestorasyonDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Mimari Restorasyon Duyurusu"
        verbose_name_plural = "Mimari Restorasyon Duyuruları"

class MimariRestorasyonEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/mimari_restorasyon/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Mimari Restorasyon)"
        verbose_name_plural = "Etkinlikler (Mimari Restorasyon)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('mimari_restorasyon:etkinlik_detay', args=[self.slug])
...
class MimariRestorasyonDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/mimari_restorasyon/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Mimari Restorasyon)"
        verbose_name_plural = "Ders Programları (Mimari Restorasyon)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MimariRestorasyonFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MimariRestorasyonFaaliyet(models.Model):
    grup = models.ForeignKey(MimariRestorasyonFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MimariRestorasyonFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MimariRestorasyonFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/mimari_restorasyon/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
