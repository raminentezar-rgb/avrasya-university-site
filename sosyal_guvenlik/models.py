# app_name: sosyal_guvenlik/models.py
...
class SosyalGuvenlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='sosyal_guvenlik',
            yayinda=True
        )

class SosyalGuvenlikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته تأمین اجتماعی"""
    
    objects = SosyalGuvenlikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Sosyal Güvenlik Duyurusu"
        verbose_name_plural = "Sosyal Güvenlik Duyuruları"

class SosyalGuvenlikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/sosyal_guvenlik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sosyal Güvenlik)"
        verbose_name_plural = "Etkinlikler (Sosyal Güvenlik)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('sosyal_guvenlik:etkinlik_detay', args=[self.slug])
...
class SosyalGuvenlikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/sosyal_guvenlik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Sosyal Güvenlik)"
        verbose_name_plural = "Ders Programları (Sosyal Güvenlik)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SosyalGuvenlikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class SosyalGuvenlikFaaliyet(models.Model):
    grup = models.ForeignKey(SosyalGuvenlikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class SosyalGuvenlikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SosyalGuvenlikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/sosyal_guvenlik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
