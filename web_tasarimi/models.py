# app_name: web_tasarimi/models.py
...
class WebTasarimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='web_tasarimi',
            yayinda=True
        )

class WebTasarimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته طراحی وب"""
    
    objects = WebTasarimiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Web Tasarımı Duyurusu"
        verbose_name_plural = "Web Tasarımı Duyuruları"

class WebTasarimiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/web_tasarimi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Web Tasarımı)"
        verbose_name_plural = "Etkinlikler (Web Tasarımı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('web_tasarimi:etkinlik_detay', args=[self.slug])
...
class WebTasarimiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/web_tasarimi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Web Tasarımı)"
        verbose_name_plural = "Ders Programları (Web Tasarımı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class WebTasarimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class WebTasarimiFaaliyet(models.Model):
    grup = models.ForeignKey(WebTasarimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class WebTasarimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(WebTasarimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/web_tasarimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
