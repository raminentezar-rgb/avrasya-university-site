# app_name: halkla_iliskiler/models.py
...
class HalklaIliskilerDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='halkla_iliskiler',
            yayinda=True
        )

class HalklaIliskilerDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته روابط عمومی"""
    
    objects = HalklaIliskilerDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Halkla İlişkiler Duyurusu"
        verbose_name_plural = "Halkla İlişkiler Duyuruları"

class HalklaIliskilerEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/halkla_iliskiler/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Halkla İlişkiler)"
        verbose_name_plural = "Etkinlikler (Halkla İlişkiler)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('halkla_iliskiler:etkinlik_detay', args=[self.slug])
...
class HalklaIliskilerDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/halkla_iliskiler/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Halkla İlişkiler)"
        verbose_name_plural = "Ders Programları (Halkla İlişkiler)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class HalklaIliskilerFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class HalklaIliskilerFaaliyet(models.Model):
    grup = models.ForeignKey(HalklaIliskilerFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class HalklaIliskilerFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(HalklaIliskilerFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/halkla_iliskiler/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
