...
class InsaatTeknolojisiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='insaat_teknolojisi',
            yayinda=True
        )

class InsaatTeknolojisiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته ساخت و ساز فناوری"""
    
    objects = InsaatTeknolojisiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İnşaat Teknolojisi Duyurusu"
        verbose_name_plural = "İnşaat Teknolojisi Duyuruları"

class InsaatTeknolojisiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/insaat_teknolojisi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İnşaat Teknolojisi)"
        verbose_name_plural = "Etkinlikler (İnşaat Teknolojisi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('insaat_teknolojisi:etkinlik_detay', args=[self.slug])
...
class InsaatTeknolojisiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/insaat_teknolojisi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İnşaat Teknolojisi)"
        verbose_name_plural = "Ders Programları (İnşaat Teknolojisi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class InsaatTeknolojisiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class InsaatTeknolojisiFaaliyet(models.Model):
    grup = models.ForeignKey(InsaatTeknolojisiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class InsaatTeknolojisiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(InsaatTeknolojisiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/insaat_teknolojisi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
