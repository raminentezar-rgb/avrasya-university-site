# app_name: saglik_kurumlari/models.py
...
class SaglikKurumlariDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='saglik_kurumlari',
            yayinda=True
        )

class SaglikKurumlariDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مدیریت مؤسسات درمانی"""
    
    objects = SaglikKurumlariDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Sağlık Kurumları Duyurusu"
        verbose_name_plural = "Sağlık Kurumları Duyuruları"

class SaglikKurumlariEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/saglik_kurumlari/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sağlık Kurumları)"
        verbose_name_plural = "Etkinlikler (Sağlık Kurumları)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('saglik_kurumlari:etkinlik_detay', args=[self.slug])
...
class SaglikKurumlariDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/saglik_kurumlari/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Sağlık Kurumları)"
        verbose_name_plural = "Ders Programları (Sağlık Kurumları)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SaglikKurumlariFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Saglik Kurumlari)"
        verbose_name_plural = "Faaliyet Grupları (Saglik Kurumlari)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class SaglikKurumlariFaaliyet(models.Model):
    grup = models.ForeignKey(SaglikKurumlariFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Saglik Kurumlari)"
        verbose_name_plural = "Faaliyetler (Saglik Kurumlari)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class SaglikKurumlariFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SaglikKurumlariFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/saglik_kurumlari/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Saglik Kurumlari)"
        verbose_name_plural = "Faaliyet Görselleri (Saglik Kurumlari)"
        ordering = ['sira', 'id']
