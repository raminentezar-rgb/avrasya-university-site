...
class SporYoneticiligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='spor-yoneticiligi',
            yayinda=True
        )

class SporYoneticiligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مدیریت ورزشی"""
...
class Spor_Etkinlik(models.Model):
    ETKINLIK_TURU_CHOICES = [
        ('konferans', 'Konferans / Kongre / Sempozyum'),
        ('seminer', 'Seminer / Panel'),
        ('turnuva', 'Spor Turnuvası'),
        ('antrenman', 'Antrenman Kampı'),
        ('sempozyum', 'Spor Bilimleri Sempozyumu'),
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/spor_afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class SporDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/spor/', verbose_name="Dosya")
