# app_name: spor_yoneticiligi/models.py
...
class SporYoneticiligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='spor_yoneticiligi',
            yayinda=True
        )


class SporYoneticiligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مدیریت ورزشی"""

    objects = SporYoneticiligiDuyuruManager()

    class Meta:
        proxy = True
        verbose_name = "Spor Yöneticiliği Duyurusu"
        verbose_name_plural = "Spor Yöneticiliği Duyuruları"


class SporYoneticiligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/spor_yoneticiligi/%Y/%m/%d/',
        blank=True,
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Spor Yöneticiliği)"
        verbose_name_plural = "Etkinlikler (Spor Yöneticiliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('spor_yoneticiligi:etkinlik_detay', args=[self.slug])
...
class SporYoneticiligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/spor_yoneticiligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Spor Yöneticiliği)"
        verbose_name_plural = "Ders Programları (Spor Yöneticiliği)"
        ordering = ['-yayin_tarihi']

    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SporYoneticiligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Spor Yoneticiligi)"
        verbose_name_plural = "Faaliyet Grupları (Spor Yoneticiligi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class SporYoneticiligiFaaliyet(models.Model):
    grup = models.ForeignKey(SporYoneticiligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Spor Yoneticiligi)"
        verbose_name_plural = "Faaliyetler (Spor Yoneticiligi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class SporYoneticiligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SporYoneticiligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/spor_yoneticiligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Spor Yoneticiligi)"
        verbose_name_plural = "Faaliyet Görselleri (Spor Yoneticiligi)"
        ordering = ['sira', 'id']
