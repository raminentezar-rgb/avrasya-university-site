# app_name: sosyal_hizmetler/models.py
...
class SosyalHizmetlerDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='sosyal_hizmetler',
            yayinda=True
        )

class SosyalHizmetlerDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته خدمات اجتماعی"""
    
    objects = SosyalHizmetlerDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Sosyal Hizmetler Duyurusu"
        verbose_name_plural = "Sosyal Hizmetler Duyuruları"

class SosyalHizmetlerEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/sosyal_hizmetler/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sosyal Hizmetler)"
        verbose_name_plural = "Etkinlikler (Sosyal Hizmetler)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('sosyal_hizmetler:etkinlik_detay', args=[self.slug])
...
class SosyalHizmetlerDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/sosyal_hizmetler/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Sosyal Hizmetler)"
        verbose_name_plural = "Ders Programları (Sosyal Hizmetler)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SosyalHizmetlerFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Sosyal Hizmetler)"
        verbose_name_plural = "Faaliyet Grupları (Sosyal Hizmetler)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class SosyalHizmetlerFaaliyet(models.Model):
    grup = models.ForeignKey(SosyalHizmetlerFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Sosyal Hizmetler)"
        verbose_name_plural = "Faaliyetler (Sosyal Hizmetler)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class SosyalHizmetlerFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SosyalHizmetlerFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/sosyal_hizmetler/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Sosyal Hizmetler)"
        verbose_name_plural = "Faaliyet Görselleri (Sosyal Hizmetler)"
        ordering = ['sira', 'id']
