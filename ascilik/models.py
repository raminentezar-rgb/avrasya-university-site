...
class AscilikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ascilik',
            yayinda=True
        )

class AscilikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته آشی‌لیک"""
    
    objects = AscilikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Aşçılık Duyurusu"
        verbose_name_plural = "Aşçılık Duyuruları"

class Ascilik_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/asciilik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Aşçılık)"
        verbose_name_plural = "Etkinlikler (Aşçılık)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('asciilik:etkinlik_detay', args=[self.slug])
...
class AscilikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/asciilik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Aşçılık)"
        verbose_name_plural = "Ders Programları (Aşçılık)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class AscilikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class AscilikFaaliyet(models.Model):
    grup = models.ForeignKey(AscilikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class AscilikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(AscilikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ascilik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
