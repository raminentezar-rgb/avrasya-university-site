...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = EgzersizEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = EgzersizEtkinlik.objects.filter(
        yayinda=True,
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = EgzersizEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__range=[timezone.now(), yedi_gun_sonra]
    ).order_by('baslangic_tarihi')
    
    return render(request, 'egzersiz/includes/etkinlikler/yaklasan.html', {
...
def ders_programi(request):
    ...
    dosyalar = EgzersizDersProgrami.objects.filter(aktif=True)
