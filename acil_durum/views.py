...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = AcilDurumEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = AcilDurumEtkinlik.objects.filter(
        yayinda=True,
    ...
    return render(request, 'acil_durum/includes/etkinlikler/list.html', context)

def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(AcilDurumEtkinlik, slug=slug, yayinda=True)
    return render(request, 'acil_durum/includes/etkinlikler/detail.html', {
        'etkinlik': etkinlik
    })
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = AcilDurumEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__range=[timezone.now(), yedi_gun_sonra]
    ).order_by('baslangic_tarihi')
    
    return render(request, 'acil_durum/includes/etkinlikler/yaklasan.html', {
        'etkinlikler': etkinlikler
    })

def acil_durum_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته اسیتژ"""
    
    # استفاده از مدل مدیر پروکسی برای فیلتر خودکار
    duyurular = AcilDurumDuyuru.objects.all().order_by('-yayin_tarihi')
    ...
    # پیدا کردن رشته اسیتژ برای نمایش اطلاعات
    acil_durum_bolumu = get_object_or_404(Bolum, kod='acil_durum', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': acil_durum_bolumu,
        'search_query': search_query,
        'tarih_query': tarih_query,
        'toplam_duyuru': duyurular.count(),
    }
    
    return render(request, 'acil_durum/includes/list.html', context)

def acil_durum_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته اسیتژ"""
    duyuru = get_object_or_404(AcilDurumDuyuru, slug=slug)
    
    # اطلاعیه‌های مرتبط
    ilgili_duyurular = AcilDurumDuyuru.objects.exclude(id=duyuru.id).order_by('-yayin_tarihi')[:3]
    
    # پیدا کردن رشته اسیتژ برای نمایش اطلاعات
    acil_durum_bolumu = get_object_or_404(Bolum, kod='acil_durum', aktif=True)
    
    return render(request, 'acil_durum/includes/detail.html', {
        'duyuru': duyuru,
...
def ders_programi(request):
    ...
    dosyalar = AcilDurumDersProgrami.objects.filter(aktif=True)
    ...
    return render(request, 'acil_durum/includes/ders_programi.html', context)
...
def toplumsal_katki(request):
    return render(request, 'acil_durum/includes/toplumsal_katki.html')