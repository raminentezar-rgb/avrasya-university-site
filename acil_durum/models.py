...
class AcilDurumDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='acil_durum',
            yayinda=True
        )

class AcilDurumDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته اسیتژ"""
    
    objects = AcilDurumDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Acil Durum Duyurusu"
        verbose_name_plural = "Acil Durum Duyuruları"

class AcilDurumEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/acil_durum/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Acil Durum)"
        verbose_name_plural = "Etkinlikler (Acil Durum)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('acil_durum:etkinlik_detay', args=[self.slug])
...
class AcilDurumDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/acil_durum/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Acil Durum)"
        verbose_name_plural = "Ders Programları (Acil Durum)"
