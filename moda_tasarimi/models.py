# app_name: moda_tasarimi/models.py
...
class ModaTasarimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='moda_tasarimi',
            yayinda=True
        )

class ModaTasarimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مد طراحی"""
    
    objects = ModaTasarimiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Moda Tasarımı Duyurusu"
        verbose_name_plural = "Moda Tasarımı Duyuruları"

class ModaTasarimiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/moda_tasarimi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Moda Tasarımı)"
        verbose_name_plural = "Etkinlikler (Moda Tasarımı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('moda_tasarimi:etkinlik_detay', args=[self.slug])
...
class ModaTasarimiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/moda_tasarimi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Moda Tasarımı)"
        verbose_name_plural = "Ders Programları (Moda Tasarımı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class ModaTasarimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Moda Tasarimi)"
        verbose_name_plural = "Faaliyet Grupları (Moda Tasarimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class ModaTasarimiFaaliyet(models.Model):
    grup = models.ForeignKey(ModaTasarimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Moda Tasarimi)"
        verbose_name_plural = "Faaliyetler (Moda Tasarimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class ModaTasarimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(ModaTasarimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/moda_tasarimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Moda Tasarimi)"
        verbose_name_plural = "Faaliyet Görselleri (Moda Tasarimi)"
        ordering = ['sira', 'id']
