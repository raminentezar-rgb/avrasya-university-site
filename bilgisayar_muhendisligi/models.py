...
class BilgisayarMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='bilgisayar-muhendisligi',
            yayinda=True
        )

class BilgisayarMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی کامپیوتر"""
...
class Bilgisayar_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/bilgisayar/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class BilgisayarDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/bilgisayar/', verbose_name="Dosya")
