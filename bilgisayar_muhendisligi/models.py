# app_name: bilgisayar_muhendisligi/models.py
...
class BilgisayarMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='bilgisayar_muhendisligi',
            yayinda=True
        )

class BilgisayarMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته کامپیوتر"""
    
    objects = BilgisayarMuhendisligiDuyuruManager()
...
class BilgisayarMuhendisligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/bilgisayar_muhendisligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Bilgisayar Mühendisliği)"
        verbose_name_plural = "Etkinlikler (Bilgisayar Mühendisliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('bilgisayar_muhendisligi:etkinlik_detay', args=[self.slug])
...
class BilgisayarMuhendisligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/bilgisayar_muhendisligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Bilgisayar Mühendisliği)"
        verbose_name_plural = "Ders Programları (Bilgisayar Mühendisliği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class BilgisayarMuhendisligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class BilgisayarMuhendisligiFaaliyet(models.Model):
    grup = models.ForeignKey(BilgisayarMuhendisligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class BilgisayarMuhendisligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(BilgisayarMuhendisligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/bilgisayar_muhendisligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
