...
class MaliyeDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='maliye',
            yayinda=True
        )

class MaliyeDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مالی"""
    
    objects = MaliyeDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Maliye Duyurusu"
        verbose_name_plural = "Maliye Duyuruları"

class MaliyeEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/maliye/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Maliye)"
        verbose_name_plural = "Etkinlikler (Maliye)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('maliye:etkinlik_detay', args=[self.slug])
...
class MaliyeDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/maliye/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Maliye)"
        verbose_name_plural = "Ders Programları (Maliye)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MaliyeFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Maliye)"
        verbose_name_plural = "Faaliyet Grupları (Maliye)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class MaliyeFaaliyet(models.Model):
    grup = models.ForeignKey(MaliyeFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Maliye)"
        verbose_name_plural = "Faaliyetler (Maliye)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class MaliyeFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MaliyeFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/maliye/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Maliye)"
        verbose_name_plural = "Faaliyet Görselleri (Maliye)"
        ordering = ['sira', 'id']
