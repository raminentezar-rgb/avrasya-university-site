# app_name: maliye/views.py

from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from duyurular.models import Duyuru, Bolum
from .models import MaliyeDuyuru, MaliyeEtkinlik, MaliyeDersProgrami, MaliyeFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = MaliyeEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = MaliyeEtkinlik.objects.filter(
        yayinda=True,
    ...
    return render(request, 'maliye/includes/etkinlikler/list.html', context)

def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(MaliyeEtkinlik, slug=slug, yayinda=True)
    return render(request, 'maliye/includes/etkinlikler/detail.html', {
        'etkinlik': etkinlik
    })
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = MaliyeEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__range=[timezone.now(), yedi_gun_sonra]
    ).order_by('baslangic_tarihi')
    
    return render(request, 'maliye/includes/etkinlikler/yaklasan.html', {
        'etkinlikler': etkinlikler
    })

def maliye_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته مالی"""
    ...
    # پیدا کردن رشته مالی برای نمایش اطلاعات
    ...
    return render(request, 'maliye/includes/list.html', context)

def maliye_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته مالی"""
    ...
    return render(request, 'maliye/includes/detail.html', {
        'duyuru': duyuru,
...
def ders_programi(request):
    ...
    dosyalar = MaliyeDersProgrami.objects.filter(aktif=True)
    ...
    return render(request, 'maliye/includes/ders_programi.html', context)

# صفحات استاتیک
def maliye_bolumu(request):
    return render(request, 'maliye/includes/maliye.html')



def kalite_yonetimi(request):
    return render(request, 'maliye/includes/kalite_yonetimi.html')

def toplumsal_katki(request):
    return render(request, 'maliye/includes/toplumsal_katki.html')
def idari_faaliyetler(request):
    gruplar = MaliyeFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'maliye/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = MaliyeFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'maliye/includes/diger_faaliyetler.html', {'gruplar': gruplar})
