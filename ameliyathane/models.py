# app_name: ameliyathane/models.py
...
class AmeliyathaneDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ameliyathane',
            yayinda=True
        )

class AmeliyathaneDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته اتاق عمل"""
    
    objects = AmeliyathaneDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Ameliyathane Duyurusu"
        verbose_name_plural = "Ameliyathane Duyuruları"

class AmeliyathaneEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ameliyathane/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Ameliyathane)"
        verbose_name_plural = "Etkinlikler (Ameliyathane)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ameliyathane:etkinlik_detay', args=[self.slug])
...
class AmeliyathaneDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/ameliyathane/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Ameliyathane)"
        verbose_name_plural = "Ders Programları (Ameliyathane)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class AmeliyathaneFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Ameliyathane)"
        verbose_name_plural = "Faaliyet Grupları (Ameliyathane)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class AmeliyathaneFaaliyet(models.Model):
    grup = models.ForeignKey(AmeliyathaneFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Ameliyathane)"
        verbose_name_plural = "Faaliyetler (Ameliyathane)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class AmeliyathaneFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(AmeliyathaneFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ameliyathane/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Ameliyathane)"
        verbose_name_plural = "Faaliyet Görselleri (Ameliyathane)"
        ordering = ['sira', 'id']
