# app_name: gastronomi_mutfak_sanatlari/models.py
...
class GastronomiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='gastronomi_mutfak_sanatlari',
            yayinda=True
        )


class GastronomiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته Gastronomi ve Mutfak Sanatları"""
    
    objects = GastronomiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Gastronomi Duyurusu"
        verbose_name_plural = "Gastronomi Duyuruları"


class GastronomiEtkinlik(models.Model):
    ETKINLIK_TURU_CHOICES = [
        ...
        ('sergi', 'Sergi'),
        ('yarisma', 'Yarışma'),
        ('yemek', 'Yemek Workshop / Tadım Etkinliği'),
        ('diger', 'Diğer'),
    ]
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/gastronomi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Gastronomi)"
        verbose_name_plural = "Etkinlikler (Gastronomi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('gastronomi:etkinlik_detay', args=[self.slug])
...
class GastronomiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/gastronomi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Gastronomi)"
        verbose_name_plural = "Ders Programları (Gastronomi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class GastronomiMutfakSanatlariFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class GastronomiMutfakSanatlariFaaliyet(models.Model):
    grup = models.ForeignKey(GastronomiMutfakSanatlariFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class GastronomiMutfakSanatlariFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(GastronomiMutfakSanatlariFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/gastronomi_mutfak_sanatlari/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
