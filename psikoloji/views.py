# app_name: psikoloji/views.py

from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from duyurular.models import Duyuru, Bolum
from .models import PsikolojiDuyuru, PsikolojiEtkinlik, PsikolojiDersProgrami, PsikolojiFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = PsikolojiEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = PsikolojiEtkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(PsikolojiEtkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = PsikolojiEtkinlik.objects.filter(
        yayinda=True,
...
def psikoloji_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته روانشناسی"""
...
def psikoloji_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته روانشناسی"""
...
def ders_programi(request):
    ...
    dosyalar = PsikolojiDersProgrami.objects.filter(aktif=True)
    ...
    return render(request, 'psikoloji/includes/ders_programi.html', context)

# صفحات استاتیک
def psikoloji_bolumu(request):
    return render(request, 'psikoloji/includes/psikoloji.html')



def kalite_yonetimi(request):
    return render(request, 'psikoloji/includes/kalite_yonetimi.html')

def toplumsal_katki(request):
    return render(request, 'psikoloji/includes/toplumsal_katki.html')
def idari_faaliyetler(request):
    gruplar = PsikolojiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'psikoloji/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = PsikolojiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'psikoloji/includes/diger_faaliyetler.html', {'gruplar': gruplar})
