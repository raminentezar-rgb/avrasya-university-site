...
class PsikolojiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='psikoloji',
            yayinda=True
        )

class PsikolojiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته روانشناسی"""
    
    objects = PsikolojiDuyuruManager()
...
class PsikolojiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/psikoloji/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Psikoloji)"
        verbose_name_plural = "Etkinlikler (Psikoloji)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('psikoloji:etkinlik_detay', args=[self.slug])
...
class PsikolojiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/psikoloji/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Psikoloji)"
        verbose_name_plural = "Ders Programları (Psikoloji)"
