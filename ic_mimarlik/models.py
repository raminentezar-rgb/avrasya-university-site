# app_name: ic_mimarlik/models.py
...
class IcMimarlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ic_mimarlik',
            yayinda=True
        )


class IcMimarlikDuyuru(Duyuru):
    """Proxy model for İç Mimarlık department announcements"""
    
    objects = IcMimarlikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İç Mimarlık Duyurusu"
        verbose_name_plural = "İç Mimarlık Duyuruları"


class IcMimarlikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ic_mimarlik/%Y/%m/%d/',
        blank=True,
        null=True,
    ...
    def get_absolute_url(self):
        return reverse('ic_mimarlik:etkinlik_detay', args=[self.slug])

    def yaklasan_etkinlik(self):
        """Checks if the event is approaching (within 7 days)"""
...
class IcMimarlikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
...
class IcMimarlikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IcMimarlikFaaliyet(models.Model):
    grup = models.ForeignKey(IcMimarlikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IcMimarlikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IcMimarlikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ic_mimarlik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
