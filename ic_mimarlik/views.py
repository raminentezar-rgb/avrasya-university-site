...
from .models import IcMimarlikDuyuru, IcMimarlik_Etkinlik, IcMimarlikDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = IcMimarlik_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = IcMimarlik_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(IcMimarlik_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = IcMimarlik_Etkinlik.objects.filter(
        yayinda=True,
...
def ic_mimarlik_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به معماری داخلی"""
    ...
    # پیدا کردن رشته معماری داخلی برای نمایش اطلاعات
    ic_mimarlik_bolumu = get_object_or_404(Bolum, kod='ic-mimarlik', aktif=True)
...
def ic_mimarlik_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه معماری داخلی"""
