...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Fizyoterapi_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Fizyoterapi_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Fizyoterapi_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Fizyoterapi_Etkinlik.objects.filter(
        yayinda=True,
...
def fizyoterapi_rehabilitasyon_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به فیزیوتراپی و توانبخشی"""
    ...
    # پیدا کردن رشته فیزیوتراپی و توانبخشی برای نمایش اطلاعات
    fizyoterapi_bolumu = get_object_or_404(Bolum, kod='fizyoterapi-ve-rehabilitasyon', aktif=True)
...
def fizyoterapi_rehabilitasyon_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه فیزیوتراپی و توانبخشی"""
...
# صفحات استاتیک
def fizyoterapi_rehabilitasyon(request):
