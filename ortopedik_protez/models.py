# app_name: ortopedik_protez/models.py
...
class OrtopedikProtezDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ortopedik_protez',
            yayinda=True
        )

class OrtopedikProtezDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته ارتوپدی پروتز"""
    
    objects = OrtopedikProtezDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Ortopedik Protez Duyurusu"
        verbose_name_plural = "Ortopedik Protez Duyuruları"

class OrtopedikProtezEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ortopedik_protez/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Ortopedik Protez)"
        verbose_name_plural = "Etkinlikler (Ortopedik Protez)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ortopedik_protez:etkinlik_detay', args=[self.slug])
...
class OrtopedikProtezDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/ortopedik_protez/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Ortopedik Protez)"
        verbose_name_plural = "Ders Programları (Ortopedik Protez)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class OrtopedikProtezFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class OrtopedikProtezFaaliyet(models.Model):
    grup = models.ForeignKey(OrtopedikProtezFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class OrtopedikProtezFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(OrtopedikProtezFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ortopedik_protez/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
