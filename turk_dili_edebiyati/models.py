...
class TurkDiliEdebiyatiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='turk-dili-ve-edebiyati',
            yayinda=True
        )

class TurkDiliEdebiyatiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های زبان و ادبیات ترکی"""
...
class Turk_Etkinlik(models.Model):
...
class TurkDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/turk/', verbose_name="Dosya")
