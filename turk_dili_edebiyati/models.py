# app_name: turk_dili_edebiyati/models.py
...
class TurkDiliEdebiyatiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='turk_dili_edebiyati',
            yayinda=True
        )

class TurkDiliEdebiyatiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته زبان و ادبیات ترکی"""
    
    objects = TurkDiliEdebiyatiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Türk Dili ve Edebiyatı Duyurusu"
        verbose_name_plural = "Türk Dili ve Edebiyatı Duyuruları"

class TurkDiliEdebiyatiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/turk_dili_edebiyati/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Türk Dili ve Edebiyatı)"
        verbose_name_plural = "Etkinlikler (Türk Dili ve Edebiyatı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('turk_dili_edebiyati:etkinlik_detay', args=[self.slug])
...
class TurkDiliEdebiyatiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/turk_dili_edebiyati/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Türk Dili ve Edebiyatı)"
        verbose_name_plural = "Ders Programları (Türk Dili ve Edebiyatı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class TurkDiliEdebiyatiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class TurkDiliEdebiyatiFaaliyet(models.Model):
    grup = models.ForeignKey(TurkDiliEdebiyatiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class TurkDiliEdebiyatiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(TurkDiliEdebiyatiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/turk_dili_edebiyati/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
