# app_name: ingiliz_dili_edebiyati/models.py
...
class IngilizDiliEdebiyatiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ingiliz_dili_edebiyati',
            yayinda=True
        )

class IngilizDiliEdebiyatiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های زبان و ادبیات انگلیسی"""
    
    objects = IngilizDiliEdebiyatiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İngiliz Dili ve Edebiyatı Duyurusu"
        verbose_name_plural = "İngiliz Dili ve Edebiyatı Duyuruları"



class Ing_Etkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İngiliz Dili ve Edebiyatı)"
        verbose_name_plural = "Etkinlikler (İngiliz Dili ve Edebiyatı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ingiliz_dili_edebiyati:etkinlik_detay', args=[self.slug])
...
class IngilizDiliEdebiyatiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IngilizDiliEdebiyatiFaaliyet(models.Model):
    grup = models.ForeignKey(IngilizDiliEdebiyatiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IngilizDiliEdebiyatiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IngilizDiliEdebiyatiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ingiliz_dili_edebiyati/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
