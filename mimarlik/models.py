...
class MimarlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='mimarlik',
            yayinda=True
        )


class MimarlikDuyuru(Duyuru):
    """پروکسی مدل برای نمایش اطلاعیه‌های رشته معماری"""
    
    objects = MimarlikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Mimarlık Duyurusu"
        verbose_name_plural = "Mimarlık Duyuruları"


class MimarlikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/mimarlik/%Y/%m/%d/',
        blank=True,
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Mimarlık)"
        verbose_name_plural = "Etkinlikler (Mimarlık)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('mimarlik:etkinlik_detay', args=[self.slug])
...
class MimarlikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
...
class MimarlikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MimarlikFaaliyet(models.Model):
    grup = models.ForeignKey(MimarlikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MimarlikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MimarlikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/mimarlik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
