# app_name: elektronorofizyoloji/models.py
...
class ElektronorofizyolojiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='elektronorofizyoloji',
            yayinda=True
        )

class ElektronorofizyolojiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته الکترونوروفیزیولوژی"""
    
    objects = ElektronorofizyolojiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Elektronorofizyoloji Duyurusu"
        verbose_name_plural = "Elektronorofizyoloji Duyuruları"

class ElektronorofizyolojiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/elektronorofizyoloji/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Elektronorofizyoloji)"
        verbose_name_plural = "Etkinlikler (Elektronorofizyoloji)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('elektronorofizyoloji:etkinlik_detay', args=[self.slug])
...
class ElektronorofizyolojiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/elektronorofizyoloji/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Elektronorofizyoloji)"
        verbose_name_plural = "Ders Programları (Elektronorofizyoloji)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class ElektronorofizyolojiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class ElektronorofizyolojiFaaliyet(models.Model):
    grup = models.ForeignKey(ElektronorofizyolojiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class ElektronorofizyolojiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(ElektronorofizyolojiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/elektronorofizyoloji/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
