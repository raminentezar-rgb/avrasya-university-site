# app_name: grafik_tasarimi/models.py
...
class GrafikTasarimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='grafik_tasarimi',
            yayinda=True
        )

class GrafikTasarimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته گرافیک طراحی"""
    
    objects = GrafikTasarimiDuyuruManager()
...
class GrafikTasarimiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/grafik_tasarimi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    def get_absolute_url(self):
        return reverse('grafik_tasarimi:etkinlik_detay', args=[self.slug])
...
class GrafikTasarimiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
...
class GrafikTasarimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class GrafikTasarimiFaaliyet(models.Model):
    grup = models.ForeignKey(GrafikTasarimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class GrafikTasarimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(GrafikTasarimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/grafik_tasarimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
