# app_name: yeni_medya_iletisim/models.py
...
class YeniMedyaIletisimDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='yeni-medya-ve-iletisim',
            yayinda=True
        )

class YeniMedyaIletisimDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های ی‌انیمرسو رسانه و ارتباطات"""
    
    objects = YeniMedyaIletisimDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Yeni Medya ve İletişim Duyurusu"
        verbose_name_plural = "Yeni Medya ve İletişim Duyuruları"

class YeniMedya_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/yeni_medya_iletisim/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Yeni Medya ve İletişim)"
        verbose_name_plural = "Etkinlikler (Yeni Medya ve İletişim)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('yeni_medya_iletisim:etkinlik_detay', args=[self.slug])
...
class YeniMedyaDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/yeni_medya_iletisim/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Yeni Medya ve İletişim)"
        verbose_name_plural = "Ders Programları (Yeni Medya ve İletişim)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class YeniMedyaIletisimFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class YeniMedyaIletisimFaaliyet(models.Model):
    grup = models.ForeignKey(YeniMedyaIletisimFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class YeniMedyaIletisimFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(YeniMedyaIletisimFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/yeni_medya_iletisim/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
