# file name: dis_protezi/models.py
...
class DisProteziDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='dis_protezi',
            yayinda=True
        )

class DisProteziDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته دندان‌سازی پروتز"""
    
    objects = DisProteziDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Diş Protezi Duyurusu"
        verbose_name_plural = "Diş Protezi Duyuruları"

class DisProteziEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/dis_protezi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Diş Protezi)"
        verbose_name_plural = "Etkinlikler (Diş Protezi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('dis_protezi:etkinlik_detay', args=[self.slug])
...
class DisProteziDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/dis_protezi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Diş Protezi)"
        verbose_name_plural = "Ders Programları (Diş Protezi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class DisProteziFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class DisProteziFaaliyet(models.Model):
    grup = models.ForeignKey(DisProteziFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class DisProteziFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(DisProteziFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/dis_protezi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
