...
from .models import GidaMuhendisligiDuyuru, Gida_Etkinlik, GidaDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Gida_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Gida_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Gida_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Gida_Etkinlik.objects.filter(
        yayinda=True,
...
def gida_muhendisligi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به مهندسی غذا"""
    ...
    # پیدا کردن رشته مهندسی غذا برای نمایش اطلاعات
    gida_bolumu = get_object_or_404(Bolum, kod='gida-muhendisligi', aktif=True)
...
def gida_muhendisligi_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه مهندسی غذا"""
    ...
    # پیدا کردن رشته مهندسی غذا برای نمایش اطلاعات
    gida_bolumu = get_object_or_404(Bolum, kod='gida-muhendisligi')
...
def ders_programi(request):
    ...
    dosyalar = GidaDersProgrami.objects.filter(aktif=True)
