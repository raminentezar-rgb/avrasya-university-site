...
class GidaMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='gida-muhendisligi',
            yayinda=True
        )

class GidaMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی غذا"""
...
class Gida_Etkinlik(models.Model):
    ETKINLIK_TURU_CHOICES = [
        ...
        ('laboratuvar', 'Laboratuvar Çalışması'),
        ('saha_gezisi', 'Saha Gezisi'),
        ('diger', 'Diğer'),
    ]
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/gida_afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    kayit_gerekiyor = models.BooleanField(default=False, verbose_name="Kayıت Gerekiyor")
...
class GidaDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    sinif = models.CharField(max_length=20, choices=SINIF_CHOICES, verbose_name="Sınıf/Düzey")
