# app_name: gida_muhendisligi/models.py
...
class GidaMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='gida_muhendisligi',
            yayinda=True
        )

class GidaMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مهندسی مواد غذایی"""
    
    objects = GidaMuhendisligiDuyuruManager()
...
class GidaMuhendisligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/gida_muhendisligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Gıda Mühendisliği)"
        verbose_name_plural = "Etkinlikler (Gıda Mühendisliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('gida_muhendisligi:etkinlik_detay', args=[self.slug])
...
class GidaMuhendisligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/gida_muhendisligi/', verbose_name="Dosya")
    ...
    class Meta:
        ...
        verbose_name_plural = "Ders Programları (Gıda Mühendisلیği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class GidaMuhendisligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class GidaMuhendisligiFaaliyet(models.Model):
    grup = models.ForeignKey(GidaMuhendisligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class GidaMuhendisligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(GidaMuhendisligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/gida_muhendisligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
