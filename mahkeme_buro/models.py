# app_name: mahkeme_buro/models.py
...
class MahkemeBuroDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='mahkeme_buro',
            yayinda=True
        )

class MahkemeBuroDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مأموریت‌های بازرگانی"""
    
    objects = MahkemeBuroDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Mahkeme Buro Duyurusu"
        verbose_name_plural = "Mahkeme Buro Duyuruları"

class MahkemeBuroEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/mahkeme_buro/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Mahkeme Buro)"
        verbose_name_plural = "Etkinlikler (Mahkeme Buro)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('mahkeme_buro:etkinlik_detay', args=[self.slug])
...
class MahkemeBuroDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/mahkeme_buro/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Mahkeme Buro)"
        verbose_name_plural = "Ders Programları (Mahkeme Buro)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MahkemeBuroFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MahkemeBuroFaaliyet(models.Model):
    grup = models.ForeignKey(MahkemeBuroFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MahkemeBuroFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MahkemeBuroFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/mahkeme_buro/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
