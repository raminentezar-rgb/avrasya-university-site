# app_name: yonetim_bilisim_sistemleri/models.py
...
class YonetimBilisimDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='yonetim_bilisim_sistemleri',
            yayinda=True
        )


class YonetimBilisimDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته Yönetim Bilişim Sistemleri"""
    
    objects = YonetimBilisimDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Yönetim Bilişim Duyurusu"
        verbose_name_plural = "Yönetim Bilişim Duyuruları"


class YonetimBilisimEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/yonetim_bilisim/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Yönetim Bilişim)"
        verbose_name_plural = "Etkinlikler (Yönetim Bilişim)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('yonetim_bilisim:etkinlik_detay', args=[self.slug])
...
class YonetimBilisimDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/yonetim_bilisim/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Yönetim Bilişim)"
        verbose_name_plural = "Ders Programları (Yönetim Bilişim)"
