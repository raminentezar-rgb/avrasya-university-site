# app_name: tibbi_goruntuleme/models.py
...
class TibbiGoruntulemeDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='tibbi_goruntuleme',
            yayinda=True
        )

class TibbiGoruntulemeDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته تیمب گورونتوله‌مه"""
    
    objects = TibbiGoruntulemeDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Tıbbi Görüntüleme Duyurusu"
        verbose_name_plural = "Tıbbi Görüntüleme Duyuruları"

class TibbiGoruntulemeEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/tibbi_goruntuleme/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Tıbbi Görüntüleme)"
        verbose_name_plural = "Etkinlikler (Tıbbi Görüntüleme)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('tibbi_goruntuleme:etkinlik_detay', args=[self.slug])
...
class TibbiGoruntulemeDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/tibbi_goruntuleme/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Tıbbi Görüntüleme)"
        verbose_name_plural = "Ders Programları (Tıbbi Görüntüleme)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class TibbiGoruntulemeFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class TibbiGoruntulemeFaaliyet(models.Model):
    grup = models.ForeignKey(TibbiGoruntulemeFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class TibbiGoruntulemeFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(TibbiGoruntulemeFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/tibbi_goruntuleme/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
