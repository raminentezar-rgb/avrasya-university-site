# app_name: beslenme_diyetetik/models.py
...
class BeslenmeDiyetetikEtkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Beslenme ve Diyetetik)"
        verbose_name_plural = "Etkinlikler (Beslenme ve Diyetetik)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('beslenme_diyetetik:etkinlik_detay', args=[self.slug])
...
class BeslenmeDiyetetikDersProgrami(models.Model):
    ...
    class Meta:
        verbose_name = "Ders Programı (Beslenme ve Diyetetik)"
        verbose_name_plural = "Ders Programları (Beslenme ve Diyetetik)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class BeslenmeDiyetetikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class BeslenmeDiyetetikFaaliyet(models.Model):
    grup = models.ForeignKey(BeslenmeDiyetetikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class BeslenmeDiyetetikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(BeslenmeDiyetetikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/beslenme_diyetetik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
