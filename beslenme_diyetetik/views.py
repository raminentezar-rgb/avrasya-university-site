# app_name: beslenme_diyetetik/views.py

from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from duyurular.models import Duyuru, Bolum
from .models import BeslenmeDiyetetikDuyuru, BeslenmeDiyetetikEtkinlik, BeslenmeDiyetetikDersProgrami, BeslenmeDiyetetikFaaliyetGrubu
...
def yaklasan_etkinlikler(request):
    ...
    return render(request, 'beslenme_diyetetik/includes/etkinlikler/yaklasan.html', {
...
def kalite_yonetimi(request):
    return render(request, 'beslenme_diyetetik/includes/kalite_yonetimi.html')

def toplumsal_katki(request):
    return render(request, 'beslenme_diyetetik/includes/toplumsal_katki.html')
def idari_faaliyetler(request):
    gruplar = BeslenmeDiyetetikFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'beslenme_diyetetik/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = BeslenmeDiyetetikFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'beslenme_diyetetik/includes/diger_faaliyetler.html', {'gruplar': gruplar})
