...
class InsaatMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='insaat-muhendisligi',
            yayinda=True
        )

class InsaatMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی عمران"""
...
class Insaat_Etkinlik(models.Model):
...
class InsaatDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/insaat/', verbose_name="Dosya")
