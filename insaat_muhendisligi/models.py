# app_name: insaat_muhendisligi/models.py
...
class InsaatMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='insaat_muhendisligi',
            yayinda=True
        )

class InsaatMuhendisligiDuyuru(Duyuru):
    """پروکسی مدل برای نمایش اطلاعیه‌های رشته مهندسی عمران"""
    
    objects = InsaatMuhendisligiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İnşaat Mühendisliği Duyurusu"
        verbose_name_plural = "İnşaat Mühendisliği Duyuruları"

class InsaatMuhendisligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/insaat_muhendisligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İnşaat Mühendisliği)"
        verbose_name_plural = "Etkinlikler (İnşaat Mühendisliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('insaat_muhendisligi:etkinlik_detay', args=[self.slug])
...
class InsaatMuhendisligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/insaat_muhendisligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İnşaat Mühendisliği)"
        verbose_name_plural = "Ders Programları (İnşaat Mühendisliği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class InsaatMuhendisligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class InsaatMuhendisligiFaaliyet(models.Model):
    grup = models.ForeignKey(InsaatMuhendisligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class InsaatMuhendisligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(InsaatMuhendisligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/insaat_muhendisligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
