...
class OtomotivDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='otomotiv',
            yayinda=True
        )


class OtomotivDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته اتوموتیو"""
    
    objects = OtomotivDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Otomotiv Duyurusu"
        verbose_name_plural = "Otomotiv Duyuruları"


class OtomotivEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/otomotiv/%Y/%m/%d/',
        blank=True,
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Otomotiv)"
        verbose_name_plural = "Etkinlikler (Otomotiv)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('otomotiv:etkinlik_detay', args=[self.slug])
...
class OtomotivDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/otomotiv/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Otomotiv)"
        verbose_name_plural = "Ders Programları (Otomotiv)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class OtomotivFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Otomotiv)"
        verbose_name_plural = "Faaliyet Grupları (Otomotiv)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class OtomotivFaaliyet(models.Model):
    grup = models.ForeignKey(OtomotivFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Otomotiv)"
        verbose_name_plural = "Faaliyetler (Otomotiv)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class OtomotivFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(OtomotivFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/otomotiv/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Otomotiv)"
        verbose_name_plural = "Faaliyet Görselleri (Otomotiv)"
        ordering = ['sira', 'id']
