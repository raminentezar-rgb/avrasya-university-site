# app_name: sosyal_hizmet/models.py
...
class SosyalHizmetDuyuru(Duyuru):
    ...
    objects = SosyalHizmetDuyuruManager()
...
class SosyalHizmetEtkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sosyal Hizmet)"
        verbose_name_plural = "Etkinlikler (Sosyal Hizmet)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('sosyal_hizmet:etkinlik_detay', args=[self.slug])
...
class SosyalHizmetDersProgrami(models.Model):
    ...
    dosya = models.FileField(upload_to='ders_programlari/sosyal_hizmet/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Sosyal Hizmet)"
        verbose_name_plural = "Ders Programları (Sosyal Hizmet)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SosyalHizmetFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class SosyalHizmetFaaliyet(models.Model):
    grup = models.ForeignKey(SosyalHizmetFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class SosyalHizmetFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SosyalHizmetFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/sosyal_hizmet/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
