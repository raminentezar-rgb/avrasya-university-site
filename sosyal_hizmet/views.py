...
from .models import SosyalHizmetDuyuru, SosyalHizmetEtkinlik, SosyalHizmetDersProgrami, SosyalHizmetFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = SosyalHizmetEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = SosyalHizmetEtkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(SosyalHizmetEtkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = SosyalHizmetEtkinlik.objects.filter(
        yayinda=True,
...
def sosyal_hizmet_duyurulari(request):
    ...
    sosyal_hizmet_bolumu = get_object_or_404(Bolum, kod='sosyal_hizmet', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': sosyal_hizmet_bolumu,
...
def sosyal_hizmet_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته مددکاری اجتماعی"""
...
def ders_programi(request):
    ...
    dosyalar = SosyalHizmetDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def sosyal_hizmet_bolumu(request):
...
def idari_faaliyetler(request):
    gruplar = SosyalHizmetFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'sosyal_hizmet/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = SosyalHizmetFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'sosyal_hizmet/includes/diger_faaliyetler.html', {'gruplar': gruplar})
