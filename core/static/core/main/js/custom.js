...
$(document).ready(function() {
    ...
    $(window).on('scroll', initMilestonesParallax);
    
    // شمارنده‌ها
    function initMilestoneCounters() {
        $('.milestone_counter').each(function() {
            var $this = $(this);
            var target = parseInt($this.data('end-value'));
            var signBefore = $this.data('sign-before') || '';
            var current = 0;
            ...
            var timer = setInterval(function() {
                ...
                $this.text(signBefore + Math.floor(current));
            }, 25);
        });
    }
    ...
    $(window).on('scroll', function() {
        var milestonesSection = $('#milestones');
        if (milestonesSection.length) {
            var windowHeight = $(window).height();
            var elementTop = milestonesSection.offset().top;
            var elementVisible = 150;
            
            if (elementTop < $(window).scrollTop() + windowHeight - elementVisible) {
                setTimeout(initMilestoneCounters, 300);
                $(window).off('scroll');
            }
        }
    });
