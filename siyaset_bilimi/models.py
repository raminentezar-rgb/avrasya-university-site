# app_name: siyaset_bilimi/models.py
...
class SiyasetBilimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='siyaset_bilimi',
            yayinda=True
        )

class SiyasetBilimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته علوم سیاسی"""
...
class SiyasetBilimiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/siyaset_bilimi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Siyaset Bilimi)"
        verbose_name_plural = "Etkinlikler (Siyaset Bilimi)"
...
class SiyasetBilimiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/siyaset_bilimi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Siyaset Bilimi)"
        verbose_name_plural = "Ders Programları (Siyaset Bilimi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SiyasetBilimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Siyaset Bilimi)"
        verbose_name_plural = "Faaliyet Grupları (Siyaset Bilimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class SiyasetBilimiFaaliyet(models.Model):
    grup = models.ForeignKey(SiyasetBilimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Siyaset Bilimi)"
        verbose_name_plural = "Faaliyetler (Siyaset Bilimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class SiyasetBilimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SiyasetBilimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/siyaset_bilimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Siyaset Bilimi)"
        verbose_name_plural = "Faaliyet Görselleri (Siyaset Bilimi)"
        ordering = ['sira', 'id']
