...
@admin.register(Mutercim_Etkinlik)
class Mutercim_EtkinlikAdmin(admin.ModelAdmin):
...
@admin.register(MutercimTercumanlikDuyuru)
class MutercimTercumanlikDuyuruAdmin(admin.ModelAdmin):
    list_display = ['baslik', 'fakulte', 'yayin_tarihi', 'yayinda']
    list_filter = ['yayinda', 'yayin_tarihi']
    search_fields = ['baslik', 'icerik']
    
    def get_queryset(self, request):
        return MutercimTercumanlikDuyuru.objects.all()

@admin.register(MutercimDersProgrami)
class MutercimDersProgramiAdmin(admin.ModelAdmin):
