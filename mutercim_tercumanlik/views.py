...
from .models import MutercimTercumanlikDuyuru, Mutercim_Etkinlik, MutercimDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Mutercim_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Mutercim_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Mutercim_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Mutercim_Etkinlik.objects.filter(
        yayinda=True,
...
def mutercim_tercumanlik_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته مترجمی"""
    
    # استفاده از مدل مدیر پروکسی برای فیلتر خودکار
    duyurular = MutercimTercumanlikDuyuru.objects.all().order_by('-yayin_tarihi')
    ...
    # پیدا کردن رشته مترجمی برای نمایش اطلاعات
    mutercim_bolumu = get_object_or_404(Bolum, kod='mutercim-tercumanlik', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': mutercim_bolumu,
...
def mutercim_tercumanlik_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته مترجمی"""
    duyuru = get_object_or_404(MutercimTercumanlikDuyuru, slug=slug)
    
    # اطلاعیه‌های مرتبط
    ilgili_duyurular = MutercimTercumanlikDuyuru.objects.exclude(id=duyuru.id).order_by('-yayin_tarihi')[:3]
    
    # پیدا کردن رشته مترجمی برای نمایش اطلاعات
    mutercim_bolumu = get_object_or_404(Bolum, kod='mutercim_tercumanlik')
...
def ders_programi(request):
    ...
    dosyalar = MutercimDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def mutercim_tercumanlik(request):
