...
class MutercimTercumanlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='mutercim-tercumanlik',
            yayinda=True
        )

class MutercimTercumanlikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مترجمی"""
    
    objects = MutercimTercumanlikDuyuruManager()
...
class Mutercim_Etkinlik(models.Model):
...
class MutercimDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/mutercim/', verbose_name="Dosya")
