# app_name: mutercim_tercumanlik/models.py
...
class MuterimTercumanlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='mutercim_tercumanlik',
            yayinda=True
        )

class MuterimTercumanlikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مترجمی و ترجمه شفاهی"""
    
    objects = MuterimTercumanlikDuyuruManager()
...
class MuterimTercumanlikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/mutercim_tercumanlik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Mütercim Tercümanlık)"
        verbose_name_plural = "Etkinlikler (Mütercim Tercümanlık)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('mutercim_tercumanlik:etkinlik_detay', args=[self.slug])
...
class MuterimTercumanlikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/mutercim_tercumanlik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Mütercim Tercümanlık)"
        verbose_name_plural = "Ders Programları (Mütercim Tercümanlık)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MutercimTercumanlikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MutercimTercumanlikFaaliyet(models.Model):
    grup = models.ForeignKey(MutercimTercumanlikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MutercimTercumanlikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MutercimTercumanlikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/mutercim_tercumanlik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
