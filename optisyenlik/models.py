# app_name: optisyenlik/models.py
...
class OptisyenlikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='optisyenlik',
            yayinda=True
        )

class OptisyenlikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته اپتومتری"""
    
    objects = OptisyenlikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Optisyenlik Duyurusu"
        verbose_name_plural = "Optisyenlik Duyuruları"

class OptisyenlikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/optisyenlik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Optisyenlik)"
        verbose_name_plural = "Etkinlikler (Optisyenlik)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('optisyenlik:etkinlik_detay', args=[self.slug])
...
class OptisyenlikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/optisyenlik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Optisyenlik)"
        verbose_name_plural = "Ders Programları (Optisyenlik)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class OptisyenlikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class OptisyenlikFaaliyet(models.Model):
    grup = models.ForeignKey(OptisyenlikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class OptisyenlikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(OptisyenlikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/optisyenlik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
