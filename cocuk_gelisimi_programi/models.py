# app_name: cocuk_gelisimi_programi/models.py
...
class CocukGelisimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='cocuk_gelisimi',
            yayinda=True
        )

class CocukGelisimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های برنامه کودک‌سازی"""
    
    objects = CocukGelisimiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Çocuk Gelişimi Duyurusu"
        verbose_name_plural = "Çocuk Gelişimi Duyuruları"

class CocukGelisimiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/cocuk_gelisimi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Çocuk Gelişimi)"
        verbose_name_plural = "Etkinlikler (Çocuk Gelişimi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('cocuk_gelisimi_programi:etkinlik_detay', args=[self.slug])
...
class CocukGelisimiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/cocuk_gelisimi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Çocuk Gelişimi)"
        verbose_name_plural = "Ders Programları (Çocuk Gelişimi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class CocukGelisimiProgramiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class CocukGelisimiProgramiFaaliyet(models.Model):
    grup = models.ForeignKey(CocukGelisimiProgramiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class CocukGelisimiProgramiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(CocukGelisimiProgramiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/cocuk_gelisimi_programi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
