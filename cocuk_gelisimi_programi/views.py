...
from .models import CocukGelisimiDuyuru, CocukGelisimiEtkinlik, CocukGelisimiDersProgrami, CocukGelisimiProgramiFaaliyetGrubu
...
def idari_faaliyetler(request):
    gruplar = CocukGelisimiProgramiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'cocuk_gelisimi_programi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = CocukGelisimiProgramiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'cocuk_gelisimi_programi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
