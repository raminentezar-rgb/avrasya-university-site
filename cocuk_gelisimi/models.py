...
class CocukGelisimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='cocuk-gelisimi',
            yayinda=True
        )

class CocukGelisimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های کودک‌گشیتی"""
...
class Cocuk_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/cocuk_gelisimi/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class CocukDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/cocuk/', verbose_name="Dosya")
