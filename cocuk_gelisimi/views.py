...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Cocuk_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Cocuk_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Cocuk_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Cocuk_Etkinlik.objects.filter(
        yayinda=True,
...
def cocuk_gelisimi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به کودک‌گشیتی"""
    ...
    # پیدا کردن رشته کودک‌گشیتی برای نمایش اطلاعات
    cocuk_bolumu = get_object_or_404(Bolum, kod='cocuk-gelisimi', aktif=True)
...
def cocuk_gelisimi_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه کودک‌گشیتی"""
...
def ders_programi(request):
    ...
    dosyalar = CocukDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def cocuk_gelisimi(request):
    return render(request, 'cocuk_gelisimi/includes/cocuk_gelisimi.html')

def idari_faaliyetler_2024_2025(request):
    return render(request, 'cocuk_gelisimi/includes/idari_faaliyetler_2024_2025.html')

def diger_faaliyetler_2024_2025(request):
    return render(request, 'cocuk_gelisimi/includes/diger_faaliyetler_2024_2025.html')
