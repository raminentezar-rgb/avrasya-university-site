# app_name: cocuk_gelisimi/views.py

from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from duyurular.models import Duyuru, Bolum
from .models import CocukGelisimiDuyuru, CocukGelisimiEtkinlik, CocukGelisimiDersProgrami, CocukGelisimiFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    return render(request, 'cocuk_gelisimi/includes/etkinlikler/list.html', context)


def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(CocukGelisimiEtkinlik, slug=slug, yayinda=True)
    return render(request, 'cocuk_gelisimi/includes/etkinlikler/detail.html', {
        'etkinlik': etkinlik
    })
...
def yaklasan_etkinlikler(request):
    ...
    return render(request, 'cocuk_gelisimi/includes/etkinlikler/yaklasan.html', {
        'etkinlikler': etkinlikler
    })


def cocuk_gelisimi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته کودک رشد"""
    ...
    # پیدا کردن رشته کودک رشد برای نمایش اطلاعات
    ...
    return render(request, 'cocuk_gelisimi/includes/list.html', context)


def cocuk_gelisimi_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته کودک رشد"""
    ...
    return render(request, 'cocuk_gelisimi/includes/detail.html', {
        'duyuru': duyuru,
...
def ders_programi(request):
    ...
    return render(request, 'cocuk_gelisimi/includes/ders_programi.html', context)


# صفحات استاتیک
def cocuk_gelisimi_bolumu(request):
    return render(request, 'cocuk_gelisimi/includes/cocuk_gelisimi.html')
...
def kalite_yonetimi(request):
    return render(request, 'cocuk_gelisimi/includes/kalite_yonetimi.html')


def toplumsal_katki(request):
    return render(request, 'cocuk_gelisimi/includes/toplumsal_katki.html')
def idari_faaliyetler(request):
    gruplar = CocukGelisimiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'cocuk_gelisimi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = CocukGelisimiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'cocuk_gelisimi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
