# app_name: bilisim_guvenligi/models.py
...
class BilisimGuvenligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='bilisim_guvenligi',
            yayinda=True
        )

class BilisimGuvenligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته امنیت اطلاعات"""
    
    objects = BilisimGuvenligiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Bilişim Güvenliği Duyurusu"
        verbose_name_plural = "Bilişim Güvenliği Duyuruları"

class BilisimGuvenligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/bilisim_guvenligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Bilişim Güvenliği)"
        verbose_name_plural = "Etkinlikler (Bilişim Güvenliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('bilisim_guvenligi:etkinlik_detay', args=[self.slug])
...
class BilisimGuvenligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/bilisim_guvenligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Bilişim Güvenliği)"
        verbose_name_plural = "Ders Programları (Bilişim Güvenliği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class BilisimGuvenligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class BilisimGuvenligiFaaliyet(models.Model):
    grup = models.ForeignKey(BilisimGuvenligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class BilisimGuvenligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(BilisimGuvenligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/bilisim_guvenligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
