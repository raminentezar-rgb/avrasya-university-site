...
class IsletmeDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='isletme',
            yayinda=True
        )

class IsletmeDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مدیریت کسب و کار"""
    
    objects = IsletmeDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İşletme Duyurusu"
        verbose_name_plural = "İşletme Duyuruları"

class IsletmeEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/isletme/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İşletme)"
        verbose_name_plural = "Etkinlikler (İşletme)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('isletme:etkinlik_detay', args=[self.slug])
...
class IsletmeDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/isletme/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İşletme)"
        verbose_name_plural = "Ders Programları (İşletme)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IsletmeFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IsletmeFaaliyet(models.Model):
    grup = models.ForeignKey(IsletmeFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IsletmeFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IsletmeFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/isletme/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
