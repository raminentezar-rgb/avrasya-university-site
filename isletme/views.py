# app_name: isletme/views.py

from django.shortcuts import render, get_object_or_404
from django.db.models import Q
from duyurular.models import Duyuru, Bolum
from .models import IsletmeDuyuru, IsletmeEtkinlik, IsletmeDersProgrami, IsletmeFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = IsletmeEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = IsletmeEtkinlik.objects.filter(
        yayinda=True,
    ...
    return render(request, 'isletme/includes/etkinlikler/list.html', context)

def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(IsletmeEtkinlik, slug=slug, yayinda=True)
    return render(request, 'isletme/includes/etkinlikler/detail.html', {
        'etkinlik': etkinlik
    })
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = IsletmeEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__range=[timezone.now(), yedi_gun_sonra]
    ).order_by('baslangic_tarihi')
    
    return render(request, 'isletme/includes/etkinlikler/yaklasan.html', {
        'etkinlikler': etkinlikler
    })

def isletme_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته مدیریت کسب و کار"""
    ...
    return render(request, 'isletme/includes/list.html', context)

def isletme_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته مدیریت کسب و کار"""
    ...
    return render(request, 'isletme/includes/detail.html', {
        'duyuru': duyuru,
...
def ders_programi(request):
    ...
    dosyalar = IsletmeDersProgrami.objects.filter(aktif=True)
    ...
    return render(request, 'isletme/includes/ders_programi.html', context)

# صفحات استاتیک
def isletme_bolumu(request):
    return render(request, 'isletme/includes/isletme.html')



def kalite_yonetimi(request):
    return render(request, 'isletme/includes/kalite_yonetimi.html')

def toplumsal_katki(request):
    return render(request, 'isletme/includes/toplumsal_katki.html')
def idari_faaliyetler(request):
    gruplar = IsletmeFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'isletme/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = IsletmeFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'isletme/includes/diger_faaliyetler.html', {'gruplar': gruplar})
