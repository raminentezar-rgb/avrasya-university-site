# app_name: gorsel_iletisim_tasarimi/models.py
...
class GorselIletisimTasarimiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='gorsel-iletisim-tasarimi',
            yayinda=True
        )

class GorselIletisimTasarimiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های طراحی ارتباط بصری"""
    
    objects = GorselIletisimTasarimiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Görsel İletişim Tasarımı Duyurusu"
        verbose_name_plural = "Görsel İletişim Tasarımı Duyuruları"

class Gorsel_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/gorsel_iletisim/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Görsel İletişim Tasarımı)"
        verbose_name_plural = "Etkinlikler (Görsel İletişim Tasarımı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('gorsel_iletisim_tasarimi:etkinlik_detay', args=[self.slug])
...
class GorselDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/gorsel_iletisim/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Görsel İletişim Tasarımı)"
        verbose_name_plural = "Ders Programları (Görsel İletişim Tasarımı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class GorselIletisimTasarimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class GorselIletisimTasarimiFaaliyet(models.Model):
    grup = models.ForeignKey(GorselIletisimTasarimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class GorselIletisimTasarimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(GorselIletisimTasarimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/gorsel_iletisim_tasarimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
