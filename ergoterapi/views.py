...
from .models import ErgoterapiDuyuru, ErgoterapiEtkinlik, ErgoterapiDersProgrami, ErgoterapiFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = ErgoterapiEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')

    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = ErgoterapiEtkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(ErgoterapiEtkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = ErgoterapiEtkinlik.objects.filter(
        yayinda=True,
...
def ergoterapi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته ارگوتراپی"""
...
def ergoterapi_duyuru_detay(request, slug):
    """نمایش جزئیات یک اطلاعیه رشته ارگوتراپی"""
...
def idari_faaliyetler(request):
    gruplar = ErgoterapiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'ergoterapi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = ErgoterapiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'ergoterapi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
