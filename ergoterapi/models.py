...
class ErgoterapiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ergoterapi',
            yayinda=True
        )


class ErgoterapiDuyuru(Duyuru):
    """پروکسی مدل برای نمایش اطلاعیه‌های رشته ارگوتراپی"""

    objects = ErgoterapiDuyuruManager()

    class Meta:
        proxy = True
        verbose_name = "Ergoterapi Duyurusu"
        verbose_name_plural = "Ergoterapi Duyuruları"


class ErgoterapiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ergoterapi/%Y/%m/%d/',
        blank=True,
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Ergoterapi)"
        verbose_name_plural = "Etkinlikler (Ergoterapi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ergoterapi:etkinlik_detay', args=[self.slug])
    ...
    def devam_ediyor(self):
        """بررسی در حال برگزاری بودن رویداد"""
        now = timezone.now()
...
class ErgoterapiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
...
class ErgoterapiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class ErgoterapiFaaliyet(models.Model):
    grup = models.ForeignKey(ErgoterapiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class ErgoterapiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(ErgoterapiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ergoterapi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
