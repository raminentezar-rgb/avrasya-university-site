...
class DiyalizDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='diyaliz',
            yayinda=True
        )

class DiyalizDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته دیالیز"""
    
    objects = DiyalizDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Diyaliz Duyurusu"
        verbose_name_plural = "Diyaliz Duyuruları"

class DiyalizEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/diyaliz/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Diyaliz)"
        verbose_name_plural = "Etkinlikler (Diyaliz)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('diyaliz:etkinlik_detay', args=[self.slug])
...
class DiyalizDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/diyaliz/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Diyaliz)"
        verbose_name_plural = "Ders Programları (Diyaliz)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class DiyalizFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Diyaliz)"
        verbose_name_plural = "Faaliyet Grupları (Diyaliz)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class DiyalizFaaliyet(models.Model):
    grup = models.ForeignKey(DiyalizFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Diyaliz)"
        verbose_name_plural = "Faaliyetler (Diyaliz)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class DiyalizFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(DiyalizFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/diyaliz/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Diyaliz)"
        verbose_name_plural = "Faaliyet Görselleri (Diyaliz)"
        ordering = ['sira', 'id']
