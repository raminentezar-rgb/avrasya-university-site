...
class IsUgrasiTerapisiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='is_ugrasi_terapisi',
            yayinda=True
        )

class IsUgrasiTerapisiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته ایی‌ی‌س‌ی تجارت خارجی"""
    
    objects = IsUgrasiTerapisiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İş Uğraşı Terapisi Duyurusu"
        verbose_name_plural = "İş Uğraşı Terapisi Duyuruları"

class IsUgrasiTerapisiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/is_ugrasi_terapisi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İş Uğraşı Terapisi)"
        verbose_name_plural = "Etkinlikler (İş Uğraşı Terapisi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('is_ugrasi_terapisi:etkinlik_detay', args=[self.slug])
...
class IsUgrasiTerapisiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/is_ugrasi_terapisi/', verbose_name="Dosya")
    sinif = models.CharField(max_length=10, choices=SINIF_CHOICES, verbose_name="Sınıf")
    yayin_tarihi = models.DateTimeField(default=timezone.now, verbose_name="Yayın Tاریخ")
    aktif = models.BooleanField(default=True, verbose_name="Aktif")
    
    class Meta:
        verbose_name = "Ders Programı (İş Uğraşı Terapisi)"
        verbose_name_plural = "Ders Programları (İş Uğraşı Terapisi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IsUgrasiTerapisiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IsUgrasiTerapisiFaaliyet(models.Model):
    grup = models.ForeignKey(IsUgrasiTerapisiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IsUgrasiTerapisiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IsUgrasiTerapisiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/is_ugrasi_terapisi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
