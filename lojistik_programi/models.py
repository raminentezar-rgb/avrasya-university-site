# app_name: lojistik_programi/models.py
...
class LojistikProgramiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='lojistik_programi',
            yayinda=True
        )

class LojistikProgramiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته لوجستیک"""
    
    objects = LojistikProgramiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Lojistik Programı Duyurusu"
        verbose_name_plural = "Lojistik Programı Duyuruları"

class LojistikProgramiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/lojistik_programi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Lojistik Programı)"
        verbose_name_plural = "Etkinlikler (Lojistik Programı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('lojistik_programi:etkinlik_detay', args=[self.slug])
...
class LojistikProgramiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/lojistik_programi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Lojistik Programı)"
        verbose_name_plural = "Ders Programları (Lojistik Programı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class LojistikProgramiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class LojistikProgramiFaaliyet(models.Model):
    grup = models.ForeignKey(LojistikProgramiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class LojistikProgramiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(LojistikProgramiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/lojistik_programi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
