# app_name: saglik_yonetimi/models.py
...
class SaglikYonetimiEtkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sağlık Yönetimi)"
        verbose_name_plural = "Etkinlikler (Sağlık Yönetimi)"
...
class SaglikYonetimiDersProgrami(models.Model):
    ...
    class Meta:
        verbose_name = "Ders Programı (Sağlık Yönetimi)"
        verbose_name_plural = "Ders Programları (Sağlık Yönetimi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SaglikYonetimiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Saglik Yonetimi)"
        verbose_name_plural = "Faaliyet Grupları (Saglik Yonetimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class SaglikYonetimiFaaliyet(models.Model):
    grup = models.ForeignKey(SaglikYonetimiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Saglik Yonetimi)"
        verbose_name_plural = "Faaliyetler (Saglik Yonetimi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class SaglikYonetimiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SaglikYonetimiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/saglik_yonetimi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Saglik Yonetimi)"
        verbose_name_plural = "Faaliyet Görselleri (Saglik Yonetimi)"
        ordering = ['sira', 'id']
