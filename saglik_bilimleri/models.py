...
class SaglikBilimleriDuyuru(models.Model):
    ...
    KATEGORI_CHOICES = [
        ...
        ('ogrenci', 'Öğrenci'),
        ('genel', 'Genel'),
        ('hastalik', 'Hastalık Duyuruları'),
        ('seminer', 'Seminer/Konferans'),
        ('staj', 'Staj Bilgileri'),
    ]
    ...
    # فایل‌های ضمیمه
    dosya = models.FileField(upload_to='saglik_bilimleri/duyurular/', blank=True, null=True, verbose_name="Dosya Eki")
    ...
    class Meta:
        verbose_name = "Sağlık Bilimleri Duyurusu"
        verbose_name_plural = "Sağlık Bilimleri Duyuruları"
    ...
    def get_absolute_url(self):
        return reverse('saglik_bilimleri:duyuru_detay', args=[self.id])
...
class SaglikBilimleriDosya(models.Model):
    """مدل برای فایل‌های ضمیمه در اپ saglik_bilimleri"""
    DOSYA_TURleri = [
        ('word', 'Word Belgesi'),
        ('pdf', 'PDF Belgesi'),
        ('excel', 'Excel Dosyası'),
    ...
    dosya = models.FileField(
        upload_to='saglik_bilimleri/dosyalar/%Y/%m/%d/',
        verbose_name="Dosya"
    )
