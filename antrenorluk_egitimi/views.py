...
from .models import AntrenorlukEgitimiDuyuru, AntrenorlukEgitimiEtkinlik, AntrenorlukEgitimiDersProgrami, AntrenorlukEgitimiFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = AntrenorlukEgitimiEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')

    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = AntrenorlukEgitimiEtkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(AntrenorlukEgitimiEtkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = AntrenorlukEgitimiEtkinlik.objects.filter(
        yayinda=True,
...
def antrenorluk_egitimi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته مربیگری آموزشی"""
    ...
    # پیدا کردن رشته مربیگری آموزشی برای نمایش اطلاعات
    antrenorluk_bolumu = get_object_or_404(Bolum, kod='antrenorluk_egitimi', aktif=True)
...
def antrenorluk_egitimi_duyuru_detay(request, slug):
    """نمایش جزئیات یک اطلاعیه رشته مربیگری آموزشی"""
...
def ders_programi(request):
    ...
    dosyalar = AntrenorlukEgitimiDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def antrenorluk_egitimi_bolumu(request):
...
def idari_faaliyetler(request):
    gruplar = AntrenorlukEgitimiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'antrenorluk_egitimi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = AntrenorlukEgitimiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'antrenorluk_egitimi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
