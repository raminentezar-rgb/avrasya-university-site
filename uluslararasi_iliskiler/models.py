...
class UluslararasiIliskilerDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='uluslararasi-iliskiler',
            yayinda=True
        )

class UluslararasiIliskilerDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های روابط بین‌الملل"""
...
class UluslararasiIliskiler_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/uluslararasi_iliskiler/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Uluslararası İlişkiler Etkinliği"
        verbose_name_plural = "Uluslararası İlişkiler Etkinlikleri"
...
class DersProgramiDosyaUluslararasiIliskiler(models.Model):
    DOSYA_TURU_CHOICES = [
        ('pdf', 'PDF'),
        ('word', 'Word'),
        ('excel', 'Excel'),
    ...
    dosya = models.FileField(
        upload_to='ders_programlari_uluslararasi_iliskiler/%Y/%m/%d/',
        verbose_name="Dosya"
    )
    ...
    class Meta:
        verbose_name = "Ders Programı Dosyası (Uluslararası İlişkiler)"
        verbose_name_plural = "Ders Programı Dosyaları (Uluslararası İlişkiler)"
