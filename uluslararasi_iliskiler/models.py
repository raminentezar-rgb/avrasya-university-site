# app_name: uluslararasi_iliskiler/models.py
...
class UluslararasiIliskilerDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='uluslararasi_iliskiler',
            yayinda=True
        )

class UluslararasiIliskilerDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته روابط بین‌الملل"""
    
    objects = UluslararasiIliskilerDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Uluslararası İlişkiler Duyurusu"
        verbose_name_plural = "Uluslararası İlişkiler Duyuruları"

class UluslararasiIliskilerEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/uluslararasi_iliskiler/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Uluslararası İlişkiler)"
        verbose_name_plural = "Etkinlikler (Uluslararası İlişkiler)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('uluslararasi_iliskiler:etkinlik_detay', args=[self.slug])
    ...
    def devam_ediyor(self):
        """Etkinliğin devam edip etmediğini کنترل eder"""
        now = timezone.now()
...
class UluslararasiIliskilerDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/uluslararasi_iliskiler/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Uluslararası İlişkiler)"
        verbose_name_plural = "Ders Programları (Uluslararası İlişkiler)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class UluslararasiIliskilerFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class UluslararasiIliskilerFaaliyet(models.Model):
    grup = models.ForeignKey(UluslararasiIliskilerFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class UluslararasiIliskilerFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(UluslararasiIliskilerFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/uluslararasi_iliskiler/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
