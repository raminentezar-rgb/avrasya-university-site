...
class AgizDisDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='agiz_dis',
            yayinda=True
        )

class AgizDisDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته بیماری‌های دهان و دندان"""
    
    objects = AgizDisDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Ağız ve Diş Sağlığı Duyurusu"
        verbose_name_plural = "Ağız ve Diş Sağlığı Duyuruları"

class AgizDisEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/agiz_dis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Ağız ve Diş Sağlığı)"
        verbose_name_plural = "Etkinlikler (Ağız ve Diş Sağlığı)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('agiz_dis:etkinlik_detay', args=[self.slug])
...
class AgizDisDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/agiz_dis/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Ağız ve Diş Sağlığı)"
        verbose_name_plural = "Ders Programları (Ağız ve Diş Sağlığı)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class AgizDisFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Agiz Dis)"
        verbose_name_plural = "Faaliyet Grupları (Agiz Dis)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class AgizDisFaaliyet(models.Model):
    grup = models.ForeignKey(AgizDisFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Agiz Dis)"
        verbose_name_plural = "Faaliyetler (Agiz Dis)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class AgizDisFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(AgizDisFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/agiz_dis/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Agiz Dis)"
        verbose_name_plural = "Faaliyet Görselleri (Agiz Dis)"
        ordering = ['sira', 'id']
