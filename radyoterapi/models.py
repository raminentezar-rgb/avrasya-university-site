# app_name: radyoterapi/models.py
...
class RadyoterapiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='radyoterapi',
            yayinda=True
        )

class RadyoterapiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته رادیوتراپی"""
    
    objects = RadyoterapiDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Radyoterapi Duyurusu"
        verbose_name_plural = "Radyoterapi Duyuruları"

class RadyoterapiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/radyoterapi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Radyoterapi)"
        verbose_name_plural = "Etkinlikler (Radyoterapi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('radyoterapi:etkinlik_detay', args=[self.slug])
...
class RadyoterapiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/radyoterapi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Radyoterapi)"
        verbose_name_plural = "Ders Programları (Radyoterapi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class RadyoterapiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class RadyoterapiFaaliyet(models.Model):
    grup = models.ForeignKey(RadyoterapiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class RadyoterapiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(RadyoterapiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/radyoterapi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
