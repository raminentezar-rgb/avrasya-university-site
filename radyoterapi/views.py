...
from .models import RadyoterapiDuyuru, RadyoterapiEtkinlik, RadyoterapiDersProgrami, RadyoterapiFaaliyetGrubu
...
def idari_faaliyetler(request):
    gruplar = RadyoterapiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'radyoterapi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = RadyoterapiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'radyoterapi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
