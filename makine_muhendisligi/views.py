...
from .models import MakineMuhendisligiDuyuru, Makine_Etkinlik, MakineDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Makine_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Makine_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Makine_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Makine_Etkinlik.objects.filter(
        yayinda=True,
...
def makine_muhendisligi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به مهندسی مکانیک"""
    ...
    # پیدا کردن رشته مهندسی مکانیک برای نمایش اطلاعات
    makine_bolumu = get_object_or_404(Bolum, kod='makine-muhendisligi', aktif=True)
...
def makine_muhendisligi_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه مهندسی مکانیک"""
    ...
    # پیدا کردن رشته مهندسی مکانیک برای نمایش اطلاعات
    makine_bolumu = get_object_or_404(Bolum, kod='makine-muhendisligi')
...
def ders_programi(request):
    ...
    dosyalar = MakineDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def makine_muhendisligi(request):
