# app_name: makine_muhendisligi/models.py
...
class MakineMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='makine_muhendisligi',
            yayinda=True
        )

class MakineMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مهندسی مکانیک"""
    
    objects = MakineMuhendisligiDuyuruManager()
...
class MakineMuhendisligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/makine_muhendisligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Makine Mühendisliği)"
        verbose_name_plural = "Etkinlikler (Makine Mühendisliği)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('makine_muhendisligi:etkinlik_detay', args=[self.slug])
...
class MakineMuhendisligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/makine_muhendisligi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Makine Mühendisliği)"
        verbose_name_plural = "Ders Programları (Makine Mühendisliği)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MakineMuhendisligiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MakineMuhendisligiFaaliyet(models.Model):
    grup = models.ForeignKey(MakineMuhendisligiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MakineMuhendisligiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MakineMuhendisligiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/makine_muhendisligi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
