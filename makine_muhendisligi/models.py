...
class MakineMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='makine-muhendisligi',
            yayinda=True
        )

class MakineMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی مکانیک"""
...
class Makine_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/makine/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class MakineDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/makine/', verbose_name="Dosya")
