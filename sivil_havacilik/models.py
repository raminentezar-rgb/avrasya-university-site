# app_name: sivil_havacilik/models.py
...
class SivilHavacilikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='sivil_havacilik',
            yayinda=True
        )

class SivilHavacilikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته هوانوردی"""
    
    objects = SivilHavacilikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Sivil Havacılık Duyurusu"
        verbose_name_plural = "Sivil Havacılık Duyuruları"

class SivilHavacilikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/sivil_havacilik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Sivil Havacılık)"
        verbose_name_plural = "Etkinlikler (Sivil Havacılık)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('sivil_havacilik:etkinlik_detay', args=[self.slug])
...
class SivilHavacilikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/sivil_havacilik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Sivil Havacılık)"
        verbose_name_plural = "Ders Programları (Sivil Havacılık)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class SivilHavacilikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class SivilHavacilikFaaliyet(models.Model):
    grup = models.ForeignKey(SivilHavacilikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class SivilHavacilikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(SivilHavacilikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/sivil_havacilik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
