...
class AdaletEtkinlikAdmin(TranslationAdmin):
...
@admin.register(AdaletDuyuru)
class AdaletDuyuruAdmin(admin.ModelAdmin):
    list_display = ['baslik', 'fakulte', 'yayin_tarihi', 'yayinda']
    list_filter = ['yayinda', 'yayin_tarihi']
    search_fields = ['baslik', 'icerik']
    
    def get_queryset(self, request):
        return AdaletDuyuru.objects.all()

@admin.register(AdaletDersProgrami)
class AdaletDersProgramiAdmin(TranslationAdmin):
...
class AdaletFaaliyetGrubuAdmin(TranslationAdmin):
    list_display = ['baslik', 'faaliyet_turu', 'sira']
    list_filter = ['faaliyet_turu']
    list_editable = ['sira']

@admin.register(AdaletFaaliyet)
class AdaletFaaliyetAdmin(TranslationAdmin):
