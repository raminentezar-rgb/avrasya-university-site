...
class OdyolojiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='odyoloji',
            yayinda=True
        )

class OdyolojiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته ادیولوژی"""
    
    objects = OdyolojiDuyuruManager()
...
class OdyolojiEtkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Odyoloji)"
        verbose_name_plural = "Etkinlikler (Odyoloji)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('odyoloji:etkinlik_detay', args=[self.slug])
...
class OdyolojiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/odyoloji/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Odyoloji)"
        verbose_name_plural = "Ders Programları (Odyoloji)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class OdyolojiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class OdyolojiFaaliyet(models.Model):
    grup = models.ForeignKey(OdyolojiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class OdyolojiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(OdyolojiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/odyoloji/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
