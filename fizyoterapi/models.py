# app_name: fizyoterapi/models.py
...
class FizyoterapiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='fizyoterapi',
            yayinda=True
        )

class FizyoterapiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته فیزیوتراپی"""
    
    objects = FizyoterapiDuyuruManager()
...
class FizyoterapiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/fizyoterapi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    def get_absolute_url(self):
        return reverse('fizyoterapi:etkinlik_detay', args=[self.slug])
...
class FizyoterapiFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Fizyoterapi)"
        verbose_name_plural = "Faaliyet Grupları (Fizyoterapi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class FizyoterapiFaaliyet(models.Model):
    grup = models.ForeignKey(FizyoterapiFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Fizyoterapi)"
        verbose_name_plural = "Faaliyetler (Fizyoterapi)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class FizyoterapiFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(FizyoterapiFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/fizyoterapi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Fizyoterapi)"
        verbose_name_plural = "Faaliyet Görselleri (Fizyoterapi)"
        ordering = ['sira', 'id']
