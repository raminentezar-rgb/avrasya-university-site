...
class ETicaretDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='e_ticaret',
            yayinda=True
        )

class ETicaretDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته تجارت الکترونیک"""
    
    objects = ETicaretDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "E-Ticaret Duyurusu"
        verbose_name_plural = "E-Ticaret Duyuruları"

class ETicaretEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/e_ticaret/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (E-Ticaret)"
        verbose_name_plural = "Etkinlikler (E-Ticaret)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('e_ticaret:etkinlik_detay', args=[self.slug])
...
class ETicaretDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/e_ticaret/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (E-Ticaret)"
        verbose_name_plural = "Ders Programları (E-Ticaret)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class ETicaretFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class ETicaretFaaliyet(models.Model):
    grup = models.ForeignKey(ETicaretFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class ETicaretFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(ETicaretFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/e_ticaret/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
