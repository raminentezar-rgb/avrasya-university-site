...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = ETicaretEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = ETicaretEtkinlik.objects.filter(
        yayinda=True,
    ...
    return render(request, 'e_ticaret/includes/etkinlikler/list.html', context)

def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(ETicaretEtkinlik, slug=slug, yayinda=True)
    return render(request, 'e_ticaret/includes/etkinlikler/detail.html', {
        'etkinlik': etkinlik
    })
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = ETicaretEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__range=[timezone.now(), yedi_gun_sonra]
    ).order_by('baslangic_tarihi')
    
    return render(request, 'e_ticaret/includes/etkinlikler/yaklasan.html', {
        'etkinlikler': etkinlikler
    })

def e_ticaret_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته تجارت الکترونیک"""
    
    # استفاده از مدل مدیر پروکسی برای فیلتر خودکار
    duyurular = ETicaretDuyuru.objects.all().order_by('-yayin_tarihi')
    ...
    # پیدا کردن رشته تجارت الکترونیک برای نمایش اطلاعات
    e_ticaret_bolumu = get_object_or_404(Bolum, kod='e_ticaret', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': e_ticaret_bolumu,
        'search_query': search_query,
        'tarih_query': tarih_query,
        'toplam_duyuru': duyurular.count(),
    }
    
    return render(request, 'e_ticaret/includes/list.html', context)

def e_ticaret_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته تجارت الکترونیک"""
    duyuru = get_object_or_404(ETicaretDuyuru, slug=slug)
    
    # اطلاعیه‌های مرتبط
    ilgili_duyurular = ETicaretDuyuru.objects.exclude(id=duyuru.id).order_by('-yayin_tarihi')[:3]
    
    # پیدا کردن رشته تجارت الکترونیک برای نمایش اطلاعات
    e_ticaret_bolumu = get_object_or_404(Bolum, kod='e_ticaret', aktif=True)
    
    return render(request, 'e_ticaret/includes/detail.html', {
        'duyuru': duyuru,
...
def ders_programi(request):
    ...
    dosyalar = ETicaretDersProgrami.objects.filter(aktif=True)
    ...
    return render(request, 'e_ticaret/includes/ders_programi.html', context)
...
def diger_faaliyetler_2024_2025(request):
    return render(request, 'e_ticaret/includes/diger_faaliyetler_2024_2025.html')
