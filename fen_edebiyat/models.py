...
class FenEdebiyatDuyuru(models.Model):
    ...
    # فایل‌های ضمیمه
    dosya = models.FileField(upload_to='fen_edebiyat/duyurular/', blank=True, null=True, verbose_name="Dosya Eki")
    ...
    def get_absolute_url(self):
        return reverse('fen_edebiyat:duyuru_detay', args=[self.id])
...
class FenEdebiyatDosya(models.Model):
    """مدل برای فایل‌های ضمیمه در اپ fen_edebiyat"""
    DOSYA_TURleri = [
        ('word', 'Word Belgesi'),
        ('pdf', 'PDF Belgesi'),
        ('excel', 'Excel Dosyası'),
    ...
    dosya = models.FileField(
        upload_to='fen_edebiyat/dosyalar/%Y/%m/%d/',
        verbose_name="Dosya"
    )
