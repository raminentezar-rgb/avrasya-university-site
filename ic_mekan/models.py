...
class IcMekanDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ic_mekan',
            yayinda=True
        )

class IcMekanDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته طراحی داخلی و معماری"""
    
    objects = IcMekanDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İç Mekan Duyurusu"
        verbose_name_plural = "İç Mekan Duyuruları"

class IcMekanEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ic_mekan/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İç Mekan)"
        verbose_name_plural = "Etkinlikler (İç Mekan)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ic_mekan:etkinlik_detay', args=[self.slug])
...
class IcMekanDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/ic_mekan/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İç Mekan)"
        verbose_name_plural = "Ders Programları (İç Mekan)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IcMekanFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Ic Mekan)"
        verbose_name_plural = "Faaliyet Grupları (Ic Mekan)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class IcMekanFaaliyet(models.Model):
    grup = models.ForeignKey(IcMekanFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Ic Mekan)"
        verbose_name_plural = "Faaliyetler (Ic Mekan)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class IcMekanFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IcMekanFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ic_mekan/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Ic Mekan)"
        verbose_name_plural = "Faaliyet Görselleri (Ic Mekan)"
        ordering = ['sira', 'id']
