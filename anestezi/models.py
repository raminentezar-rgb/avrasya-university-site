...
class AnesteziDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='anestezi',
            yayinda=True
        )

class AnesteziDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته بیهوشی"""
    
    objects = AnesteziDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Anestezi Duyurusu"
        verbose_name_plural = "Anestezi Duyuruları"

class AnesteziEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/anestezi/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Anestezi)"
        verbose_name_plural = "Etkinlikler (Anestezi)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('anestezi:etkinlik_detay', args=[self.slug])
...
class AnesteziDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/anestezi/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Anestezi)"
        verbose_name_plural = "Ders Programları (Anestezi)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class AnesteziFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class AnesteziFaaliyet(models.Model):
    grup = models.ForeignKey(AnesteziFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class AnesteziFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(AnesteziFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/anestezi/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
