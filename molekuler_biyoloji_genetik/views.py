...
from .models import MolekulerBiyolojiGenetikDuyuru, MolekulerBiyolojiGenetik_Etkinlik, MolekulerBiyolojiGenetikDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = MolekulerBiyolojiGenetik_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = MolekulerBiyolojiGenetik_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(MolekulerBiyolojiGenetik_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = MolekulerBiyolojiGenetik_Etkinlik.objects.filter(
        yayinda=True,
...
def molekuler_biyoloji_genetik_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به زیست شناسی مولکولی و ژنتیک"""
    ...
    # پیدا کردن رشته زیست شناسی مولکولی و ژنتیک برای نمایش اطلاعات
    molekuler_bolumu = get_object_or_404(Bolum, kod='molekuler-biyoloji-ve-genetik', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': molekuler_bolumu,
...
def molekuler_biyoloji_genetik_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه زیست شناسی مولکولی و ژنتیک"""
    ...
    # پیدا کردن رشته زیست شناسی مولکولی و ژنتیک برای نمایش اطلاعات
    molekuler_bolumu = get_object_or_404(Bolum, kod='molekuler-biyoloji-ve-genetik')
...
# صفحات استاتیک
def molekuler_biyoloji_genetik(request):
