# app_name: molekuler_biyoloji_genetik/models.py
...
class MolekulerBiyolojiGenetikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='molekuler_biyoloji_genetik',
            yayinda=True
        )

class MolekulerBiyolojiGenetikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مولکولی بیولوژی و ژنتیک"""
    
    objects = MolekulerBiyolojiGenetikDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Moleküler Biyoloji ve Genetik Duyurusu"
        verbose_name_plural = "Moleküler Biyoloji ve Genetik Duyuruları"

class MolekulerBiyolojiGenetikEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/molekuler_biyoloji_genetik/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    def get_absolute_url(self):
        return reverse('molekuler_biyoloji_genetik:etkinlik_detay', args=[self.slug])
...
class MolekulerBiyolojiGenetikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/molekuler_biyoloji_genetik/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Moleküler Biyoloji ve Genetik)"
        verbose_name_plural = "Ders Programları (Moleküler Biyoloji ve Genetik)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class MolekulerBiyolojiGenetikFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class MolekulerBiyolojiGenetikFaaliyet(models.Model):
    grup = models.ForeignKey(MolekulerBiyolojiGenetikFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class MolekulerBiyolojiGenetikFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(MolekulerBiyolojiGenetikFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/molekuler_biyoloji_genetik/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
