...
class MolekulerBiyolojiGenetikDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='molekuler-biyoloji-ve-genetik',
            yayinda=True
        )

class MolekulerBiyolojiGenetikDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های زیست شناسی مولکولی و ژنتیک"""
...
class MolekulerBiyolojiGenetik_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis_molekuler/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class MolekulerBiyolojiGenetikDersProgrami(models.Model):
    ...
    dosya = models.FileField(upload_to='ders_programlari/molekuler/', verbose_name="Dosya")
