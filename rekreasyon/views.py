...
from .models import RekreasyonDuyuru, RekreasyonEtkinlik, RekreasyonDersProgrami, RekreasyonFaaliyetGrubu
...
def rekreasyon_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به رشته تفریحات"""
    ...
    # پیدا کردن رشته تفریحات برای نمایش اطلاعات
...
def rekreasyon_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه رشته تفریحات"""
...
def idari_faaliyetler(request):
    gruplar = RekreasyonFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'rekreasyon/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = RekreasyonFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'rekreasyon/includes/diger_faaliyetler.html', {'gruplar': gruplar})
