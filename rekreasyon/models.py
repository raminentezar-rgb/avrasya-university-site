...
class RekreasyonEtkinlik(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Rekreasyon)"
        verbose_name_plural = "Etkinlikler (Rekreasyon)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('rekreasyon:etkinlik_detay', args=[self.slug])
...
class RekreasyonDersProgrami(models.Model):
    ...
    dosya = models.FileField(upload_to='ders_programlari/rekreasyon/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Rekreasyon)"
        verbose_name_plural = "Ders Programları (Rekreasyon)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class RekreasyonFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class RekreasyonFaaliyet(models.Model):
    grup = models.ForeignKey(RekreasyonFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class RekreasyonFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(RekreasyonFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/rekreasyon/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
