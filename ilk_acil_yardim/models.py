# app_name: ilk_acil_yardim/models.py
...
class IlkAcilYardimDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='ilk_acil_yardim',
            yayinda=True
        )

class IlkAcilYardimDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته İlk ve Acil Yardım"""
    
    objects = IlkAcilYardimDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "İlk ve Acil Yardım Duyurusu"
        verbose_name_plural = "İlk ve Acil Yardım Duyuruları"

class IlkAcilYardimEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/ilk_acil_yardim/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (İlk ve Acil Yardım)"
        verbose_name_plural = "Etkinlikler (İlk ve Acil Yardım)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('ilk_acil_yardim:etkinlik_detay', args=[self.slug])
...
class IlkAcilYardimDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/ilk_acil_yardim/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (İlk ve Acil Yardım)"
        verbose_name_plural = "Ders Programları (İlk ve Acil Yardım)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class IlkAcilYardimFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
...
class IlkAcilYardimFaaliyet(models.Model):
    grup = models.ForeignKey(IlkAcilYardimFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
...
class IlkAcilYardimFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(IlkAcilYardimFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/ilk_acil_yardim/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
