...
class HaritaMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='harita-muhendisligi',
            yayinda=True
        )

class HaritaMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی نقشه‌برداری"""
...
class Harita_Etkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/harita/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class HaritaDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
