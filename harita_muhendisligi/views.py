...
from .models import HaritaMuhendisligiDuyuru, HaritaMuhendisligiEtkinlik, HaritaMuhendisligiDersProgrami, HaritaMuhendisligiFaaliyetGrubu
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = HaritaMuhendisligiEtkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # Past events (before now)
    gecmis_etkinlikler = HaritaMuhendisligiEtkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(HaritaMuhendisligiEtkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = HaritaMuhendisligiEtkinlik.objects.filter(
        yayinda=True,
...
def harita_muhendisligi_duyurulari(request):
    """Display all announcements for Harita Mühendisliği department"""
    ...
    # Find Harita Mühendisliği department
    harita_muhendisligi_bolumu = get_object_or_404(Bolum, kod='harita_muhendisligi', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': harita_muhendisligi_bolumu,
...
def harita_muhendisligi_duyuru_detay(request, slug):
    """Display details of a Harita Mühendisliği announcement"""
...
def ders_programi(request):
    # Display only active files
    dosyalar = HaritaMuhendisligiDersProgrami.objects.filter(aktif=True)
...
# Static pages
def harita_muhendisligi_bolumu(request):
...
def idari_faaliyetler(request):
    gruplar = HaritaMuhendisligiFaaliyetGrubu.objects.filter(faaliyet_turu='idari').prefetch_related('faaliyetler__gorseller')
    return render(request, 'harita_muhendisligi/includes/idari_faaliyetler.html', {'gruplar': gruplar})

def diger_faaliyetler(request):
    gruplar = HaritaMuhendisligiFaaliyetGrubu.objects.filter(faaliyet_turu='diger').prefetch_related('faaliyetler__gorseller')
    return render(request, 'harita_muhendisligi/includes/diger_faaliyetler.html', {'gruplar': gruplar})
