...
class EczaneDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='eczane',
            yayinda=True
        )

class EczaneDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته داروسازی"""
    
    objects = EczaneDuyuruManager()
    
    class Meta:
        proxy = True
        verbose_name = "Eczane Duyurusu"
        verbose_name_plural = "Eczane Duyuruları"

class EczaneEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/eczane/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    class Meta:
        ...
        verbose_name = "Etkinlik (Eczane)"
        verbose_name_plural = "Etkinlikler (Eczane)"

    def __str__(self):
        return self.baslik

    def get_absolute_url(self):
        return reverse('eczane:etkinlik_detay', args=[self.slug])
...
class EczaneDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    dosya = models.FileField(upload_to='ders_programlari/eczane/', verbose_name="Dosya")
    ...
    class Meta:
        verbose_name = "Ders Programı (Eczane)"
        verbose_name_plural = "Ders Programları (Eczane)"
        ordering = ['-yayin_tarihi']
    
    def __str__(self):
        return f"{self.baslik} - {self.get_sinif_display()}"

class EczaneFaaliyetGrubu(models.Model):
    TUR_CHOICES = [
    ...
    class Meta:
        verbose_name = "Faaliyet Grubu (Eczane)"
        verbose_name_plural = "Faaliyet Grupları (Eczane)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return f"{self.baslik} ({self.get_faaliyet_turu_display()})"

class EczaneFaaliyet(models.Model):
    grup = models.ForeignKey(EczaneFaaliyetGrubu, on_delete=models.CASCADE, related_name='faaliyetler', verbose_name="Faaliyet Grubu")
    ...
    class Meta:
        verbose_name = "Faaliyet (Eczane)"
        verbose_name_plural = "Faaliyetler (Eczane)"
        ordering = ['sira', '-id']
        
    def __str__(self):
        return self.baslik

class EczaneFaaliyetGorseli(models.Model):
    faaliyet = models.ForeignKey(EczaneFaaliyet, on_delete=models.CASCADE, related_name='gorseller', verbose_name="Faaliyet")
    gorsel = models.ImageField(upload_to=f'faaliyetler/eczane/', verbose_name="Görsel")
    sira = models.IntegerField(default=0, verbose_name="Sıralama")
    
    class Meta:
        verbose_name = "Faaliyet Görseli (Eczane)"
        verbose_name_plural = "Faaliyet Görselleri (Eczane)"
        ordering = ['sira', 'id']
