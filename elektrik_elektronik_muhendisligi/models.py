...
class ElektrikElektronikMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='elektrik_elektronik_muhendisligi',
            yayinda=True
        )

class ElektrikElektronikMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های رشته مهندسی برق و الکترونیک"""
...
class ElektrikElektronikMuhendisligiEtkinlik(models.Model):
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/afis/elektrik_elektronik_muhendisligi/%Y/%m/%d/', 
        blank=True, 
        null=True,
...
class ElektrikElektronikMuhendisligiDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
