# app_name: elektrik_elektronik_muhendisligi/models.py
...
class ElektrikElektronikMuhendisligiDuyuruManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(
            bolumler__kod='elektrik-elektronik-muhendisligi',
            yayinda=True
        )

class ElektrikElektronikMuhendisligiDuyuru(Duyuru):
    """مدل پروکسی برای نمایش اطلاعیه‌های مهندسی برق و الکترونیک"""
    
    objects = ElektrikElektronikMuhendisligiDuyuruManager()
...
class Elektrik_Etkinlik(models.Model):
    ETKINLIK_TURU_CHOICES = [
        ...
        ('proje', 'Proje Yarışması / Hackathon'),
    ...
    afis = models.ImageField(
        upload_to='etkinlikler/elektrik/afis/%Y/%m/%d/', 
        blank=True, 
        null=True,
    ...
    def get_absolute_url(self):
        return reverse('elektrik_elektronik_muhendisligi:etkinlik_detay', args=[self.slug])
...
class ElektrikDersProgrami(models.Model):
    SINIF_CHOICES = [
        ('1', '1. Sınıf'),
        ('2', '2. Sınıf'),
        ('3', '3. Sınıf'),
    ...
    sinif = models.CharField(max_length=20, choices=SINIF_CHOICES, verbose_name="Sınıf")
