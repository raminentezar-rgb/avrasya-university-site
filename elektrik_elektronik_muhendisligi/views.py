...
from .models import ElektrikElektronikMuhendisligiDuyuru, Elektrik_Etkinlik, ElektrikDersProgrami
...
def etkinlik_listesi(request):
    ...
    gelecek_etkinlikler = Elektrik_Etkinlik.objects.filter(
        yayinda=True,
        baslangic_tarihi__gte=now
    ).order_by('baslangic_tarihi')
    
    # رویدادهای گذشته (قبل از حالا)
    gecmis_etkinlikler = Elektrik_Etkinlik.objects.filter(
        yayinda=True,
...
def etkinlik_detay(request, slug):
    etkinlik = get_object_or_404(Elektrik_Etkinlik, slug=slug, yayinda=True)
...
def yaklasan_etkinlikler(request):
    ...
    etkinlikler = Elektrik_Etkinlik.objects.filter(
        yayinda=True,
...
def elektrik_elektronik_muhendisligi_duyurulari(request):
    """نمایش تمام اطلاعیه‌های مربوط به مهندسی برق و الکترونیک"""
    ...
    elektrik_bolumu = get_object_or_404(Bolum, kod='elektrik-elektronik-muhendisligi', aktif=True)
    
    context = {
        'duyurular': duyurular,
        'bolum': elektrik_bolumu,
...
def elektrik_elektronik_muhendisligi_duyuru_detay(request, slug):
    """نمایش جزییات یک اطلاعیه مهندسی برق و الکترونیک"""
    ...
    # پیدا کردن رشته مهندسی برق و الکترونیک برای نمایش اطلاعات
    elektrik_bolumu = get_object_or_404(Bolum, kod='elektrik-elektronik-muhendisligi')
...
def ders_programi(request):
    ...
    dosyalar = ElektrikDersProgrami.objects.filter(aktif=True)
...
# صفحات استاتیک
def elektrik_elektronik_muhendisligi(request):
